...
async function openHome(page: Page) {
  ...
  await expect(
    page.getByRole("heading", { name: /Trending Now/i }).first()
...
test.describe("ScrollableSection — native scroll rail", () => {
  ...
  test("every rail-scroll container has overflow-x: auto", async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      const overflowX = await rails.nth(i).evaluate((el) =>
  ...
  test("at least one rail-scroll container has scrollable content (scrollWidth > clientWidth)", async ({
    ...
    let overflowCount = 0;
    for (let i = 0; i < count; i++) {
      const isScrollable = await rails.nth(i).evaluate(
        (el) => el.scrollWidth > el.clientWidth
      );
      if (isScrollable) overflowCount++;
    }
    expect(overflowCount, "expected at least one rail to have overflowing content").toBeGreaterThan(0);
  });
  ...
  test("overflowing rail accepts scrollLeft changes (native scroll is not blocked)", async ({
    ...
    for (let i = 0; i < count; i++) {
      const hasOverflow = await rails.nth(i).evaluate(
        (el) => el.scrollWidth > el.clientWidth
      );
      if (hasOverflow) {
  ...
  test("scroll-right arrow button advances scrollLeft on desktop", async ({
    page,
  }) => {
...
test.describe("ScrollableSection — mobile viewport", () => {
  ...
  test("rail containers are scrollable on mobile (at least one overflows)", async ({
    page,
  }) => {
    const rails = page.locator(".rail-scroll");
    const count = await rails.count();

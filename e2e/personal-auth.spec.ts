...
test.describe("Personal User Authentication", () => {
  ...
  test.describe("Registration", () => {
    ...
    test("shows duplicate email error for existing account", async ({ page }) => {
      ...
      await expect(page.getByText(/account already exists|email already registered/i)).toBeVisible({
        timeout: 15000,
      });
    });
    ...
    test("switch to login from duplicate email error screen", async ({ page }) => {
      ...
      await page.getByRole("button", { name: /create account/i }).click();
      await expect(page.getByRole("button", { name: /switch to login/i })).toBeVisible({
        timeout: 15000,
      });

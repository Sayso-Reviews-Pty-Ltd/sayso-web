...
test.describe("Business Goes Live Validation", () => {
  ...
  test("TC009 - Approved Business Public Accessibility", async ({ page }) => {
    ...
    if (await descriptionField.isVisible()) {
      await descriptionField.fill(
        "Premium dining experience at the V&A Waterfront with ocean views and contemporary cuisine."
      );
    }
    ...
    const businessResponse = await page.request.get(
      `${baseURL}/api/businesses?search=${encodeURIComponent(liveBusinessName)}`
    );
  ...
  test("TC010 - Business Discoverability and Search Integration", async ({ page }) => {
    ...
    const businessName = await page
      .evaluate(() => window.liveBusinessName)
      .catch(() => "E2E Live Test Business");
    ...
    const newBusinessesSection = page.locator("section", {
      has: page.getByText(/new.*business|latest.*business/i),
    });
    if (await newBusinessesSection.isVisible()) {
      await expect(newBusinessesSection.getByText(businessName, { exact: false })).toBeVisible({
        timeout: 5000,
      });
    }
    ...
    if (businessSlug) {
      ...
      if (await shareButton.isVisible()) {
        ...
        const socialOptions = page.locator(
          '[href*="facebook.com"], [href*="twitter.com"], [href*="linkedin.com"]'
        );
  ...
  test("TC011 - Live Business Review Functionality", async ({ page }) => {
    ...
    if (await ratingButtons.nth(3).isVisible()) {
      // 4/5 stars (0-indexed)
    ...
    await reviewTextarea.fill(
      "Great new business! Excellent food and service. Highly recommended for anyone visiting the V&A Waterfront."
    );
    ...
    await expect(page.getByText(/review.*submitted|thank you|success/i)).toBeVisible({
      timeout: 15000,
    });

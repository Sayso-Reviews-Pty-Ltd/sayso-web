...
test.describe("Business Upload and Submission", () => {
  ...
  test("TC003 - Valid Business Upload and Submission", async ({ page }) => {
    ...
    const subcategoryDropdown = page.locator(
      '[placeholder*="subcategory"], #subcategory, [name="subcategory"]'
    );
    await subcategoryDropdown.click();
    await page
      .getByText(/restaurant|cafe/i)
      .first()
      .click();
    ...
    await expect(page.getByText(/submitted.*review|submitted.*approval|success/i)).toBeVisible({
      timeout: 15000,
    });
    ...
    const businessCard = page.locator(".business-card", {
      has: page.getByText(uniqueBusinessName),
    });
    ...
    const businessResponse = await page.request.get(
      `${baseURL}/api/businesses?search=${encodeURIComponent(uniqueBusinessName)}`
    );
    if (businessResponse.ok()) {
      const businessData = await businessResponse.json();
      const publicBusiness = businessData.businesses?.find(
        (b: any) => b.name === uniqueBusinessName && b.status === "live"
      );
  ...
  test("TC005 - Multiple Business Submissions by Same Owner", async ({ page }) => {
    ...
    const firstBusinessCard = page.locator(".business-card", {
      has: page.getByText(firstBusinessName),
    });
    const secondBusinessCard = page.locator(".business-card", {
      has: page.getByText(secondBusinessName),
    });

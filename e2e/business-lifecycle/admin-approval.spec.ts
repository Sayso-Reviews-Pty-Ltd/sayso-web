...
test.describe("Admin Review and Approval Process", () => {
  ...
  test("TC006 - Business Appears in Admin Pending Queue", async ({ page }) => {
    ...
    await businessRow
      .getByRole("button", { name: /review|view|details/i })
      .first()
      .click();
  ...
  test("TC007 - Admin Approves Business Successfully", async ({ page }) => {
    ...
    if (!testBusinessName) {
      ...
      await page
        ...
        .click();
      await page
        .locator('[placeholder*="subcategory"], #subcategory, [name="subcategory"]')
        .click();
      await page
        .getByText(/restaurant/i)
        .first()
    ...
    await expect(page.getByText(/approved.*success|business.*approved/i)).toBeVisible({
      timeout: 15000,
    });
    ...
    await page.goto(`${baseURL}/admin/pending-businesses`);
    await expect(
      page.locator("table tbody tr").filter({ hasText: testBusinessName })
    ).not.toBeVisible({ timeout: 5000 });

    // 10. Check business status updated to 'approved' in admin list
    await page.goto(`${baseURL}/admin/businesses`);
    const approvedBusinessRow = page
      .locator("table tbody tr")
      .filter({ hasText: testBusinessName });
    ...
    const businessResponse = await page.request.get(
      `${baseURL}/api/businesses?search=${encodeURIComponent(testBusinessName)}`
    );
  ...
  test("TC008 - Admin Rejects Business with Reason", async ({ page }) => {
    ...
    const confirmRejectButton = page.getByRole("button", {
      name: /confirm.*reject|reject|disapprove/i,
    });
    ...
    await page.goto(`${baseURL}/admin/businesses`);
    const rejectedBusinessRow = page
      .locator("table tbody tr")
      .filter({ hasText: rejectionBusinessName });
    ...
    await page.goto(`${baseURL}/admin/pending-businesses`);
    await expect(
      page.locator("table tbody tr").filter({ hasText: rejectionBusinessName })
    ).not.toBeVisible({ timeout: 5000 });
    ...
    const ownerBusinessCard = page.locator(".business-card", {

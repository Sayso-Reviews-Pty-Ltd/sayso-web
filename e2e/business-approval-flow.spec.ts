...
test.describe("Business approval flow", () => {
  ...
  test("create business → pending → admin approve → business is live and visible", async ({
    ...
    await expect(page)
      .toHaveURL(/\/admin\/pending-businesses/, { timeout: 8000 })
      .catch(() => {});
    ...
    const list2 =
      listData2?.businesses ?? listData2?.data ?? (Array.isArray(listData2) ? listData2 : []);

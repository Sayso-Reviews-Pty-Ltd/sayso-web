...
test.describe("Messaging flows", () => {
  ...
  test("user starts chat with business and sends message", async ({ page }) => {
    ...
    const conversation = (conversationsPayload?.data || []).find(
      (item: any) => item.business_id === BUSINESS_ID
    );
    ...
    const hasMessage = (messagesPayload?.data?.messages || []).some(
      (item: any) => item.body === messageText
    );
  ...
  test("business replies and user receives message in realtime", async ({ browser }) => {
    ...
    try {
      ...
      await expect(userPage.getByPlaceholder("Type a message...").first()).toBeVisible({
        timeout: 20_000,
      });
      ...
      await businessPage.goto(
        `/my-businesses/messages?business_id=${BUSINESS_ID}&conversation=${conversationId}`
      );

...
test.describe('Reset Password', () => {
  ...
  test('shows invalid link screen when visiting without a token', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /invalid link/i })).toBeVisible({ timeout: 10000 });
    await expect(page.getByText('This reset link is no longer valid', { exact: true })).toBeVisible();
  });

  test('invalid link screen shows Request new link button', async ({ page }) => {
    await page.goto(`${BASE_URL}/reset-password`);

    await expect(page.getByRole('button', { name: /request new link/i })).toBeVisible({ timeout: 10000 });
  });
  ...
  test('invalid link screen shows Back to Login button', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /back to login/i })).toBeVisible({ timeout: 10000 });
  });
  ...
  test('shows invalid link screen for a malformed or expired code in URL', async ({ page }) => {
    await page.goto(`${BASE_URL}/auth/callback?code=invalid-fake-token&type=recovery&next=/reset-password`);
  ...
  test.describe('Password change (admin API)', () => {
    ...
    test('changes password via admin API and new password works on login page', async ({ page }) => {

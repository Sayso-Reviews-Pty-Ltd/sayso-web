...
export default function DMPage() {
  ...
  if (authLoading || !user || (role === "business" && businessesLoading)) {
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-off-white">
        <div className="inline-flex items-center gap-2 text-sm text-charcoal/60 font-urbanist">
  ...
  return (
    <>
      ...
      <div className="min-h-[100dvh] bg-off-white font-urbanist">
        <m.div
        ...
        <m.div {...getChoreoItemMotion({ order: 1, intent: "section", enabled: choreoEnabled })}>
          <MessagingWorkspace
            role={role}
            title={role === "business" ? "Inbox" : "Messages"}
            subtitle={
              role === "business" ? "Manage customer conversations" : "Message businesses directly"
            }

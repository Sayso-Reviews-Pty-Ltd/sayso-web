  ...
  try {
    ...
    return await fetchAllRows<BusinessRow>((from, to) =>
      supabase
        .from("businesses")
        .select(
          "slug, primary_category_slug, primary_subcategory_slug, location, primary_subcategory_label, updated_at, created_at"
        )
        .eq("status", "active")
...
async function getEvents(): Promise<
  Array<{ id: string; updated_at: string | null; created_at: string | null }>
> {
  try {
    const supabase = getSitemapSupabase();
    const today = new Date().toISOString().split("T")[0];

    return await fetchAllRows<{ id: string; updated_at: string | null; created_at: string | null }>(
      (from, to) =>
        supabase
...
async function getPublicProfiles(): Promise<
  Array<{ user_id: string; updated_at: string | null; created_at: string | null }>
> {
  try {
    const supabase = getSitemapSupabase();
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const subcategoryUrls: MetadataRoute.Sitemap = Array.from(subcategoryMap.entries()).map(
    ([path, lastModified]) => ({
      url: `${SITE_URL}${path}`,
      lastModified,
      changeFrequency: "daily" as const,
  ...
  for (const row of businessRows) {
    ...
    const category =
      typeof row.primary_subcategory_label === "string" ? row.primary_subcategory_label : "";
  ...
  const cityUrls: MetadataRoute.Sitemap = Array.from(citySlugMap.entries()).map(
    ([slug, dates]) => ({
      url: `${SITE_URL}/${slug}`,

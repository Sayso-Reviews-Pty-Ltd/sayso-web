...
function mergeRepresentativeFromOccurrences(
  ...
  for (const row of occurrences) {
    ...
    merged.event_description =
      pickLongerText(merged.event_description, row.event_description) ?? null;
    ...
    merged.event_last_modified =
      pickMaxDate(merged.event_last_modified, row.event_last_modified) ?? null;

    if (
      isNonEmptyArray(row.quicket_categories_json) &&
      !isNonEmptyArray(merged.quicket_categories_json)
    ) {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isQuicketEvent(repRow)) {
      ...
      const categoryNames =
        repRow.quicket_category_label &&
        repRow.quicket_category_label.trim().toLowerCase() !== "community"
          ? [repRow.quicket_category_label]
          : [];
    ...
    if (occMissingOptionalColumns) {
      console.warn(
        "[events-and-specials/[id]] optional columns missing; retrying occurrences with base fields."
      );

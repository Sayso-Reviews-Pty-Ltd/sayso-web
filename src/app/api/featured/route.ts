...
function getFeaturedPeriod(date: Date) {
  ...
  const weekNumber =
    1 + Math.round((tempDate.getTime() - firstThursday.getTime()) / (7 * 24 * 60 * 60 * 1000));
...
function buildFeaturedReason(business: any, categoryLabel: string) {
  ...
  return {
    label: `Sayso Select for ${category}`,
    metric: "featured_score",
    value: Number(business.featured_score ?? 0),
  };
}
...
async function getFeaturedFallback(
  ...
  const scored = businesses
    .map((business) => {
      ...
      if (
        ...
        lastActivityTime < freshnessCutoff
      ) {
        return null;
      }
      ...
      const bayesianRating =
        (averageRating * totalReviews + priorRating * priorReviews) / (totalReviews + priorReviews);
      const score =
        bayesianRating * 0.6 + Math.log(1 + totalReviews) * 0.2 + Math.log(1 + recent30) * 0.2;
      ...
      const isLocal =
        !!region && typeof business.location === "string"
  ...
  const combined = [...winners, ...sorted.filter((business) => !usedIds.has(business.id))].slice(
    0,
    limit
  );
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[FEATURED API] Service role not configured, using anon (guests may get empty if RLS restricts)"
      );
    }
    ...
    if (!coldStartError && Array.isArray(coldStartRows) && coldStartRows.length > 0) {
      ...
      console.warn(
        "[FEATURED API] Cold-start RPC error, falling back to stats RPC:",
        coldStartError.message
      );
    }
      ...
      if (diversified.length < limit) {
        ...
        for (const b of rest) {
          ...
          usedSubcategories.add(
            (b.sub_interest_id ?? b.category ?? "misc").toString().trim().toLowerCase()
          );
        }
      }
    ...
    const missingCoordinateIds = featuredData
      .filter(
        (business: any) => typeof business?.lat !== "number" || typeof business?.lng !== "number"
      )
    ...
    const imagesByBusinessId: Record<
      string,
      Array<{ business_id: string; url: string; is_primary: boolean | null }>
    > = {};
    ...
    const featuredBusinesses = featuredData.map((business: any, index: number) => {
      ...
      const slug = (business.sub_interest_id || business.category || business.bucket || "")
        .toString()
        .trim()
        .toLowerCase();
      const dbLabel =
        business.category_label && typeof business.category_label === "string"
      ...
      const interestId =
        business.interest_id || getInterestIdForSubcategory(slug) || "miscellaneous";

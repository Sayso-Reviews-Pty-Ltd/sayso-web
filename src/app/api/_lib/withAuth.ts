...
function createBearerSupabase(accessToken: string): ServerSupabase {
...
async function resolveAuthenticatedUser(
  req: NextRequest
): Promise<{ user: User | null; supabase: ServerSupabase }> {
  const bearerToken = extractBearerToken(req);

  // Mobile/native path: bearer token first (falls back to cookie session if invalid/missing).
  if (bearerToken) {
    try {
      const bearerSupabase = createBearerSupabase(bearerToken);
      const {
        data: { user },
        error,
      ...
      // Ignore bearer parse/validation issues and attempt cookie fallback below.
    }
  }

  const cookieSupabase = await getServerSupabase(req);
  const {
    data: { user },
  } = await cookieSupabase.auth.getUser();

  return { user: user ?? null, supabase: cookieSupabase };
}

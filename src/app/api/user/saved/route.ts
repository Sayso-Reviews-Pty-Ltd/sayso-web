    ...
    const businesses = savedRecords
      .map((savedRecord: any) => {
        ...
        return {
          ...
          image:
            cover_image ||
            (uploaded_images && uploaded_images.length > 0 ? uploaded_images[0] : null) ||
            business.image_url ||
            null,
...
export const POST = withUser(async (req: NextRequest, { user, supabase }) => {
  try {
    ...
    if (existing) {
      console.log("Business already saved:", {
        user_id: user.id,
        business_id,
        existing_id: existing.id,
      });
      return NextResponse.json(
        { success: true, message: "Business already saved", isSaved: true },
        { status: 200 }
      );
    }
    ...
    if (testReadError) {
      console.warn(
        "POST /api/user/saved - Warning: Cannot read from saved_businesses table (RLS issue?):",
        {
      ...
      console.log(
        "POST /api/user/saved - RLS read test passed, can read",
        testRead?.length || 0,
        "existing records"
      );
    }
    ...
    if (saveError) {
      ...
      if (saveError.code === "23505") {
        console.log("Business already saved (unique constraint):", {
          user_id: user.id,
          business_id,
        });
        return NextResponse.json(
      ...
      if (isTableError) {
        ...
        return NextResponse.json(
          {
            error:
              "Saved businesses feature is not available. The table may not exist or you may not have permission.",
            details: errorDetails.message,
            code: errorDetails.code,
          },
          { status: 500 }
    ...
    if (verifyError) {
      ...
      console.warn(
        "Verification query failed, but insert succeeded. Record may exist but RLS prevented read:",
        {
          user_id: user.id,
          business_id,
          saved_record: saved,
    ...
    if (!verifySaved) {
      console.error(
        "CRITICAL: Business insert reported success but record not found in database:",
        {
          user_id: user.id,
          business_id,
          saved_record: saved,
      ...
      console.warn(
        "Record not found in verification query, but insert succeeded. Returning success with insert data."
      );
      return NextResponse.json({
        success: true,
        message: "Business saved successfully",

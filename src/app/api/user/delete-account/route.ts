...
async function removeFromBucket(bucket: string, paths: string[], context: string) {
  if (paths.length === 0) return;
  const service = getServiceSupabase();
  const { error } = await service.storage.from(bucket).remove(paths);
  if (error) {
    console.error(
      `Error deleting ${context} from storage (continuing with account deletion):`,
      error
    );
    return;
  }
...
export const DELETE = withUser(async (_req: NextRequest, { user, supabase }) => {
  try {
    ...
    try {
      const { data: reviews } = await service.from("reviews").select("id").eq("user_id", user.id);
    ...
    try {
      ...
      if (claims && claims.length > 0) {
        ...
        const [{ data: docsV2 }, { data: docsV1 }] = await Promise.all([
          service.from("business_claim_documents").select("storage_path").in("claim_id", claimIds),
          service.from("claim_documents").select("storage_path").in("claim_id", claimIds),
        ]);

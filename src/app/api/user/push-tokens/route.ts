...
export const POST = withUser(async (req: NextRequest, { user, supabase }) => {
  try {
    ...
    const { data, error } = await (supabase as any)
      ...
      .select(
        "id, user_id, expo_push_token, platform, device_id, app_version, last_seen_at, disabled_at"
      )
      .single();
...
export const DELETE = withUser(async (req: NextRequest, { user, supabase }) => {
  try {
    ...
    if (!expoPushToken && !deviceId) {
      return NextResponse.json({ error: "expoPushToken or deviceId is required" }, { status: 400 });
    }

...
// Images are compressed client-side to ≤1 MB before submission.
// The per-file limit here is a server-side safety net only.
const MAX_IMAGES = 2;
const MAX_IMAGE_SIZE = 2 * 1024 * 1024; // 2 MB safety net (client compresses to ≤1 MB)
  ...
  try {
    ...
    if (imageIdsToRemove.length > 0) {
      ...
      const imagesToRemove =
        currentImages?.filter((img) => imageIdsToRemove.includes(img.id)) || [];
    ...
    if (imageFiles.length > 0) {
      ...
      const newImageCount =
        action === "replace" ? imageFiles.length : remainingImages + imageFiles.length;
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        // Get public URL
        const { data: urlData } = supabase.storage.from("review_images").getPublicUrl(filePath);

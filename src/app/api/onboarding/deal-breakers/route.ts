...
export const POST = withUser(async (req: NextRequest, { user, supabase }) => {
  try {
    ...
    const rawDealbreakers = dealbreakers.map((id: unknown) =>
    ...
    if (dealbreakersError) {
      console.warn(
        "[Dealbreakers API] RPC failed, falling back to direct writes:",
        dealbreakersError
      );
      ...
      const { error: insertError } = await supabase.from("user_dealbreakers").insert(rows);
    ...
    return addNoCacheHeaders(
      NextResponse.json(
        { error: "Failed to save dealbreakers", message: error.message },
        { status: 500 }
      )
    );
  }

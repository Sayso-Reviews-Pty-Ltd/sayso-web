...
export const POST = withUser(async (req: NextRequest, { user, supabase }) => {
  try {
    ...
    if (emptySubcategories.length > 0) {
      return addNoCacheHeaders(
        NextResponse.json({ error: "All subcategories must be non-empty strings" }, { status: 400 })
      );
    }
    ...
    if (subcategoryData.length === 0) {
      return addNoCacheHeaders(
        NextResponse.json({ error: "No valid subcategories provided" }, { status: 400 })
      );
    }
    ...
    if (rpcResult.error) {
      console.warn(
        "[Subcategories API] RPC failed, falling back to direct writes:",
      ...
      if (deleteError) {
        ...
        const errResponse = NextResponse.json(
          { error: "Failed to save subcategories", message: deleteError.message },
          { status: 500 }
        );
      ...
      if (insertError) {
        ...
        const errResponse = NextResponse.json(
          { error: "Failed to save subcategories", message: insertError.message },
          { status: 500 }
        );
        return addNoCacheHeaders(errResponse);
      }
      ...
      if (updateError) {
        ...
        const errResponse = NextResponse.json(
          { error: "Failed to save subcategories", message: updateError.message },
          { status: 500 }
        );
        return addNoCacheHeaders(errResponse);
      }

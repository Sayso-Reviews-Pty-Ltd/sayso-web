...
export const POST = withUser(async (_req: NextRequest, { user, supabase }) => {
  ...
  try {
    ...
    if (interestsResult.error) {
      console.error("[Complete API] Error checking interests:", interestsResult.error);
      throw interestsResult.error;
    }
    if (!interestsResult.data || interestsResult.data.length === 0)
      return addNoCacheHeaders(
        NextResponse.json({ error: "Complete interests first" }, { status: 400 })
      );
    if (subcategoriesResult.error) {
      console.error("[Complete API] Error checking subcategories:", subcategoriesResult.error);
    ...
    if (dealbreakersResult.error) {
      console.error("[Complete API] Error checking dealbreakers:", dealbreakersResult.error);
      throw dealbreakersResult.error;
    }
    if (!dealbreakersResult.data || dealbreakersResult.data.length === 0)
      return addNoCacheHeaders(
        NextResponse.json({ error: "Complete dealbreakers first" }, { status: 400 })
      );

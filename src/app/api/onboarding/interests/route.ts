...
export const POST = withUser(async (req: NextRequest, { user, supabase }) => {
  try {
    ...
    if (!interests || !Array.isArray(interests)) {
      const response = NextResponse.json({ error: "Interests must be an array" }, { status: 400 });
    ...
    if (normalizedInterests.length > 6) {
      const response = NextResponse.json({ error: "Please select 3-6 interests" }, { status: 400 });
      return addNoCacheHeaders(response);
    }

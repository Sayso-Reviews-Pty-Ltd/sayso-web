...
export const POST = withAdmin(async (req, { user, service, params }) => {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "A valid rejection reason is required. Choose one: Duplicate, Incomplete information, Inappropriate content, Other.",
        },
        { status: 400 }
      );
    }
    ...
    const fullReason = body.comment ? `${reason}: ${body.comment.trim()}` : reason;

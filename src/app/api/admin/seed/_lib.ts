...
function parseRow(
  row: SeedInputRow,
  rowNumber: number
): { parsed: ParsedSeedRow | null; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
  const warnings: string[] = [];

  const name = toOptionalString(getFieldValue(row, "name")) || "";
  const location = toOptionalString(getFieldValue(row, "location")) || "";
  const primarySubcategorySlugRaw =
    toOptionalString(getFieldValue(row, "primary_subcategory_slug")) || "";
  ...
  if (verifiedRaw === "invalid")
    errors.push("Invalid verified value (use TRUE/FALSE, 1/0, yes/no)");

  const isHiddenRaw = parseBooleanLike(getFieldValue(row, "is_hidden"));
  if (isHiddenRaw === "invalid")
    errors.push("Invalid is_hidden value (use TRUE/FALSE, 1/0, yes/no)");

  const isSystemRaw = parseBooleanLike(getFieldValue(row, "is_system"));
  if (isSystemRaw === "invalid")
    errors.push("Invalid is_system value (use TRUE/FALSE, 1/0, yes/no)");
  ...
  const primaryCategorySlugResult = normalizePrimaryCategorySlug(
    getFieldValue(row, "primary_category_slug")
  );
...
async function fetchCanonicalSlugMap(
  service: any,
  slugs: string[]
): Promise<Map<string, CanonicalSlugRecord>> {
...
async function fetchExistingNormalizedNames(
  service: any,
  parsedRows: ParsedSeedRow[]
): Promise<Map<string, { id: string }>> {
  const map = new Map<string, { id: string }>();
...
export async function validateSeedRows(params: {
  ...
  for (const row of provisionalRows) {
    ...
    row.parsed.primary_category_slug =

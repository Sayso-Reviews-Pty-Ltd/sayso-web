...
export const GET = withUser(async (_req: NextRequest, { user, supabase }) => {
  try {
    ...
    if (businessesResult.error)
      console.error("[Saved Count API] Error fetching businesses count:", businessesResult.error);
    if (eventsResult.error)
      console.error("[Saved Count API] Error fetching events count:", eventsResult.error);
    if (specialsResult.error)
      console.error("[Saved Count API] Error fetching specials count:", specialsResult.error);

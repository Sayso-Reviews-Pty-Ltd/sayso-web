...
export async function GET(req: Request) {
  try {
    ...
    const earnedBadgeMap = new Map(earnedBadges?.map((b) => [b.badge_id, b.awarded_at]) || []);
    ...
    const grouped = {
      explorer: badgesWithStatus.filter(
        (b) => b.badge_group === "explorer" || b.badge_group === "category_explorer"
      ),
      specialist: badgesWithStatus.filter(
        (b) => b.badge_group === "specialist" || b.badge_group === "category_specialist"
      ),
      milestone: badgesWithStatus.filter((b) => b.badge_group === "milestone"),
      community: badgesWithStatus.filter(
        (b) => b.badge_group === "community" || b.badge_group === "personality"
      ),
    };

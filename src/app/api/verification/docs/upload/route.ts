...
export const POST = withUser(async (req: NextRequest, { user }) => {
  try {
    ...
    const claimId =
      formData.get("claimId")?.toString()?.trim() ?? formData.get("claim_id")?.toString()?.trim();
    const docType =
      formData.get("docType")?.toString()?.trim() ?? formData.get("doc_type")?.toString()?.trim();
    ...
    if (!DOC_TYPES.includes(docType as (typeof DOC_TYPES)[number])) {
      return NextResponse.json(
        { error: "docType must be letterhead_authorization or lease_first_page" },
        { status: 400 }
      );
    }
    ...
    if (claimRow.claimant_user_id !== user.id) {
      return NextResponse.json(
        { error: "You can only upload documents for your own claim" },
        { status: 403 }
      );
    }

...
export const POST = withUser(async (req: NextRequest, { user }) => {
  try {
    ...
    if (autoMode) {
      ...
      if (!autoResult.ok) {
        const status =
          autoResult.code === "NOT_FOUND" ? 404 : autoResult.code === "INVALID_STATUS" ? 409 : 500;
    ...
    if (otpRow.code_hash !== codeHash) {
      return otpVerifyError(400, "OTP_INVALID", "Invalid code", {
    ...
    if (!moveResult.ok) {
      const status =
        moveResult.code === "NOT_FOUND" ? 404 : moveResult.code === "INVALID_STATUS" ? 409 : 500;
      return otpVerifyError(
        status,
        "OTP_VERIFICATION_FAILED",

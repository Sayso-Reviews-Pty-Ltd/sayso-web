...
export const POST = withUser(async (_req: NextRequest, { user, supabase, params }) => {
  try {
    ...
    const senderUserIdToMarkRead =
      access.role === "user" ? access.conversation.owner_id : access.conversation.user_id;
    ...
    if (modernUpdateError && isMessageSchemaDriftError(modernUpdateError)) {
      ...
      const { data: legacyUpdatedRows, error: legacyUpdateError } =
        await legacyUpdateQuery.select("id");

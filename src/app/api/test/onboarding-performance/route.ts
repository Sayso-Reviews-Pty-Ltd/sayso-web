...
export async function GET() {
  ...
  try {
    ...
    let { data: profile, error: profileError } = await supabase
      .from("profiles")
      .select(
        "onboarding_step, onboarding_complete, onboarding_completed_at, interests_count, subcategories_count, dealbreakers_count"
      )
      .eq("user_id", user.id)
      .single();
    ...
    if (profileError && isSchemaCacheError(profileError)) {
      ({ data: profile, error: profileError } = await supabase
        .from("profiles")
        .select(
          "onboarding_step, onboarding_complete, interests_count, subcategories_count, dealbreakers_count"
        )
        .eq("user_id", user.id)
        .single());
    }

...
export const POST = withUser(async (req: NextRequest, { user, supabase, params }) => {
  try {
      ...
      return NextResponse.json(
        {
          ...
          details: businessError?.message || "Business does not exist",
      ...
      return NextResponse.json(
        {
          ...
          details: "User is not the owner of this business",
    ...
    if (newUrls.length === 0) {
      return NextResponse.json({ error: "Valid image URLs are required" }, { status: 400 });
    }

...
import { applyAnonymousCookie, resolveAnonymousId } from "../../../../lib/utils/anonymousReviews";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!authError && user) {
      ...
      return NextResponse.json({ recorded: !insertError }, { status: 200 });
    }

    // Guest viewer path (deduped by anonymous_id + day).
    const { anonymousId, setCookie } = resolveAnonymousId(req);
    const { error: guestInsertError } = await supabase.from("business_profile_guest_views").insert({
      business_id: businessId,
    ...
    const response = NextResponse.json({ recorded: !guestInsertError }, { status: 200 });
    if (setCookie) {

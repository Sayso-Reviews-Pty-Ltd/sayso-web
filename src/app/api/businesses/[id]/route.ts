...
import {
  ...
  fetchBusinessOptimized,
} from "../../../lib/utils/optimizedQueries";
  ...
  try {
    ...
    if (!businessIdentifier || businessIdentifier.trim() === "") {
      ...
      return applyPrivateCachePolicy(
        NextResponse.json({ error: "Business ID or slug is required" }, { status: 400 })
      );
    }
    ...
    if (business.status !== "active") {
      ...
      try {
        ...
        if (user) {
          isOwnerOrAdmin = business.owner_id === user.id || (await isAdmin(user.id));
        }
      } catch {
    ...
    const categoryLabel =
      business.primary_subcategory_label ?? business.category_label ?? undefined;
    const interestSlug = business.primary_category_slug ?? business.interest_id ?? undefined;
    const resolvedInterestId =
      interestSlug ?? (categorySlug ? getInterestIdForSubcategory(categorySlug) : undefined);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { name, description, category, address, phone, email, website, priceRange, hours } = body;
  ...
  try {
    ...
    if (businessImages && businessImages.length > 0) {
      ...
      if (storagePaths.length > 0) {
        ...
        if (storageError) {
          console.warn(
            "[API] Error deleting storage files (continuing with business deletion):",
            storageError
          );
          // Continue with business deletion even if storage deletion fails
        } else {
          console.log(
            `[API] Deleted ${storagePaths.length} storage files for business ${actualBusinessId}`
          );
        }
      }
    }

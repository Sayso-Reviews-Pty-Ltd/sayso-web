...
import {
  getInterestIdForSubcategory,
  LEGACY_TRAVEL_SUBCATEGORY_MAP,
} from "../../../lib/onboarding/subcategoryMapping";
...
export const POST = withAdmin(async (req: NextRequest, { service: supabase }) => {
  try {
    ...
    const normalizedKey =
      typeof rawSubcategory === "string" ? rawSubcategory.trim().toLowerCase() : undefined;
    ...
    const subcategoriesToSeed = subcategory ? [subcategory] : ALL_SUBCATEGORIES;

    console.log(
      `[SEED] Fetching up to ${limit} businesses from ${subcategoriesToSeed.length} subcategory/subcategories...`
    );
    ...
    for (let i = 0; i < subcategoriesToSeed.length; i++) {
      ...
      const businessesPerSubcategory =
        subcategoriesRemaining <= 1
          ? remaining
      ...
      try {
        console.log(
          `[SEED] Fetching up to ${businessesPerSubcategory} businesses for subcategory: ${subcat} (${i + 1}/${subcategoriesToSeed.length})`
        );
        const subcategoryBusinesses = await fetchCapeTownBusinesses(
          businessesPerSubcategory,
          subcat
        );
    ...
    const generateUniqueSlug = (name: string, sourceId: string): string => {
      ...
      const sourceHashBase = sourceId.toLowerCase().replace(/[^a-z0-9]/g, "");
    ...
    mappedBusinesses.forEach((business, index) => {
      ...
      if (seen.has(key)) {
        ...
        console.warn(
          `[SEED] Skipping duplicate business: ${business.name} (${key}) - duplicate of index ${duplicateIndex}`
        );
    ...
    console.log(
      `[SEED] Deduplicated ${mappedBusinesses.length} businesses to ${uniqueBusinesses.length} unique businesses`
    );
    ...
    if (duplicates.length > 0) {
      ...
      const trulyUnique = uniqueBusinesses.filter((business) => {
        ...
        return !duplicates.some(
          (d) => d.source === business.source && d.source_id === business.source_id
        );
      });
      console.log(
        `[SEED] Removed ${duplicates.length} duplicates, final count: ${trulyUnique.length}`
      );
    ...
    for (let i = 0; i < sourceIds.length; i += lookupBatchSize) {
      ...
      if (existingBatchError) {
        console.warn(
          "[SEED] Warning: Failed to check existing businesses batch:",
    ...
    const businessesToInsert = uniqueBusinesses.filter(
      (business) => !existingSourceIds.has(business.source_id)
    );
    const alreadyExistingCount = uniqueBusinesses.length - businessesToInsert.length;

    console.log(
      `[SEED] ${businessesToInsert.length} new businesses to insert (${alreadyExistingCount} already existed in database)`
    );
    ...
    console.log(
      `[SEED] Inserting ${businessesToInsert.length} businesses in batches of ${batchSize}...`
    );
    ...
    for (let i = 0; i < businessesToInsert.length; i += batchSize) {
      ...
      const batch = rawBatch.map((b: Record<string, unknown>) => {
        ...
        return {
          ...
          primary_subcategory_label:
            (SUBCATEGORY_SLUG_TO_LABEL[slug as keyof typeof SUBCATEGORY_SLUG_TO_LABEL] ??
              categoryLabel) ||
            "Miscellaneous",
      ...
      console.log(
        `[SEED] Inserting batch ${batchNumber}/${totalBatches} (${batch.length} businesses)...`
      );
      ...
      if (batchError) {
        ...
        if (batchError.code === "23505") {
          ...
          console.error(
            `[SEED] Duplicate key error in batch ${batchNumber}. Checking for duplicates...`
          );
          ...
          batch.forEach((b: Record<string, unknown>, idx: number) => {
            ...
            if (batchKeys.has(key)) {
              console.error(
                `[SEED] Duplicate in batch ${batchNumber} at index ${idx}: ${b.name} - ${key}`
              );
            } else {
              batchKeys.add(key);
            }
        ...
        console.log(
          `[SEED] Successfully inserted batch ${batchNumber}/${totalBatches} (${batchData.length} businesses)`
        );
      }
    }
    ...
    if (insertedBusinesses.length > 0) {
      console.log(
        `[SEED] Successfully inserted ${insertedBusinesses.length} out of ${businessesToInsert.length} businesses`
      );
      if (insertError) {
    ...
    if (statsToInsert.length > 0) {
      const { error: statsError } = await supabase.from("business_stats").upsert(statsToInsert, {
...
export const GET = withAdmin(async (req: NextRequest) => {
  try {
    ...
    const rawSubcategory =
      searchParams.get("subcategory") || searchParams.get("category") || undefined;
    ...
    const subcategoriesToPreview = subcategory ? [subcategory] : ALL_SUBCATEGORIES;

    console.log(
      `[SEED PREVIEW] Fetching up to ${limit} businesses from ${subcategoriesToPreview.length} subcategory/subcategories...`
    );
    ...
    for (let i = 0; i < subcategoriesToPreview.length; i++) {
      ...
      try {
        console.log(
          `[SEED PREVIEW] Fetching up to ${businessesPerSubcategory} businesses for subcategory: ${subcat} (${i + 1}/${subcategoriesToPreview.length})`
        );
        const subcategoryBusinesses = await fetchCapeTownBusinesses(
          businessesPerSubcategory,
          subcat
        ...
        console.warn(
          `[SEED PREVIEW] Failed to fetch businesses for subcategory ${subcat}:`,
          error.message
        );
        // Continue with other subcategories
      }

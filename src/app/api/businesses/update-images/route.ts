...
export const POST = withAdmin(async (req: NextRequest, { service: supabase }) => {
  try {
    ...
    if (!businesses || businesses.length === 0) {
      return NextResponse.json({ message: "No businesses found", updated: 0 }, { status: 200 });
    }
    ...
    for (let i = 0; i < businesses.length; i += batchSize) {
      ...
      for (const business of batch) {
        ...
        const subcategorySlug =
          (business as { primary_subcategory_slug?: string }).primary_subcategory_slug ||
      ...
      console.log(
        `[UPDATE-IMAGES] Processed ${Math.min(i + batchSize, businesses.length)}/${businesses.length} businesses...`
      );
    }

    console.log(
      `[UPDATE-IMAGES] Update complete! Updated: ${updated}, Skipped: ${skipped}, Errors: ${errors.length}`
    );

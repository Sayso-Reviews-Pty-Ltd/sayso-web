  ...
  try {
    ...
    if (rpcError) {
      ...
      const { data: fallbackData, error: fallbackError } = await supabase
        .from("businesses")
        .select(
          "id, name, primary_subcategory_slug, primary_subcategory_label, primary_category_slug, location, address, phone, email, website, hours, image_url, verified, lat, lng, slug, is_system, is_hidden"
        )
        .eq("status", "active")
        .or("is_hidden.is.null,is_hidden.eq.false")
        .or("is_system.is.null,is_system.eq.false")
        .or(
          `name.ilike.%${query}%, description.ilike.%${query}%, primary_subcategory_slug.ilike.%${query}%, primary_subcategory_label.ilike.%${query}%`
        )
        .limit(limit);
    ...
    if (userId) {
      ...
      const ownedBusinessIds = new Set((ownedBusinesses || []).map((o) => o.business_id));
      ...
      const pendingBusinessIds = new Set((pendingRequests || []).map((r) => r.business_id));
      ...
      const claimedBusinessIds = new Set((allOwners || []).map((o) => o.business_id));

      // Build results with claim status
      for (const business of businesses) {
        const ownership = ownershipByBusinessId.get(business.id);
        const isOwnedByUser =
          ownedBusinessIds.has(business.id) ||
          (ownership?.owner_id != null && ownership.owner_id === userId);
        const hasPendingByUser = pendingBusinessIds.has(business.id);
        const isClaimedByOwnerFlags =
          Boolean(ownership?.owner_verified) || ownership?.owner_id != null;
        ...
        const subInterestId =
          (business as SearchBusinessesResult & { primary_subcategory_slug?: string })
        ...
        const interestId =
          (business as SearchBusinessesResult & { primary_category_slug?: string })
            .primary_category_slug ??
          business.interest_id ??
          (subInterestId ? getInterestIdForSubcategory(subInterestId) : undefined);
        const categorySlug =
          (business as SearchBusinessesResult & { primary_subcategory_slug?: string })
            .primary_subcategory_slug ?? (business as { category?: string }).category;
        const categoryLabel =
          (business as SearchBusinessesResult & { primary_subcategory_label?: string })
            .primary_subcategory_label ??
          subInterestLabel ??
          getSubcategoryLabel(categorySlug ?? "") ??
          categorySlug;
      ...
      for (const business of businesses) {
        ...
        results.push({
          ...
          verified: business.verified,
          claim_status:
            claimedBusinessIds.has(business.id) || isClaimedByOwnerFlags ? "claimed" : "unclaimed",
    ...
    const responseBusinesses: BusinessSearchResult[] = rankedResults.map(
      ({ hours: _hours, ...rest }) => rest
    );

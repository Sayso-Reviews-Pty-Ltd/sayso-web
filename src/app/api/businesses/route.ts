...
export async function GET(req: Request) {
  ...
  try {
    ...
    try {
      serviceSupabase = getServiceSupabase() as unknown as Awaited<
    ...
    const requestUrl =
      req?.url ?? `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/businesses`;
    ...
    if (feedStrategy === "mixed" && !userId) {
      console.log("FOR_YOU GUEST FALLBACK", {
        message: "No user — falling back to standard feed.",
      });
    ...
    if (feedStrategy === "mixed") {
      ...
      const seedWindow = explicitSeed
        ? {
            seed: explicitSeed,
            expiresAtMs: Date.now() + seedWindowMinutes * 60_000,
      ...
      const preferenceSource =
        interestIds.length > 0 || subInterestIds.length > 0 ? "client" : userId ? "server" : "none";
      ...
      let resolvedSubInterestIds =
        subInterestIds.length > 0 ? subInterestIds : subcategoriesToFilter;
      ...
      if (interestIds.length === 0 && subInterestIds.length === 0) {
        ...
        resolvedDealbreakerIds =
          dealbreakerIds.length > 0 ? dealbreakerIds : userPrefs.dealbreakerIds;
        ...
        resolvedSubInterestIds =
          userPrefs.subcategoryIds.length > 0 ? userPrefs.subcategoryIds : mappedSubcategories;
      }
      ...
      if (resolvedInterestIds.length === 0 && resolvedSubInterestIds.length === 0) {
        ...
        return withDuration(
          createForYouErrorResponse({
            ...
            code: "FOR_YOU_PREFS_MISSING",
            message:
              "No onboarding preferences found. Please complete onboarding to personalize For You.",
            requestId,
          })
        );
      }
    ...
    const isTimeoutOrAbort =
      error?.name === "AbortError" || /timeout|timed out|abort/i.test(String(error?.message ?? ""));
    if (isTimeoutOrAbort) {
      console.warn(
        "[BUSINESSES API] GET timed out / aborted:",
        error?.message ?? error,
        "total ms:",
        totalMs
      );
...
export async function HEAD(req: Request) {
  try {
    ...
    const transformedBusinesses = excludeSystemBusinesses((data || []) as any[]).map(
      (business: any) => {
        ...
        return {
          ...
          rating:
            business.average_rating > 0 ? Math.round(business.average_rating * 2) / 2 : undefined,
...
export async function POST(req: Request) {
  try {
    ...
    let errorMessage =
      "We encountered an unexpected error while creating your business listing. Please try again in a moment.";

...
function getCardBusinessBucket(business: Record<string, unknown>): string {
  return normalizeDiversityBucket(
...
export async function handleForYouFeed(options: MixedFeedOptions): Promise<NextResponse> {
  ...
  try {
    ...
    const fetchLimit = Math.min(
      Math.max(cursorOffset + limit + 1, cursorOffset + limit * 5 + 5, limit + 1),
      1000
    );
  ...
  if (filteredOutAll) {
    console.warn(
      "[BUSINESSES API] Stat-dependent dealbreakers removed all For You results; relaxing.",
      {
  ...
  if (
    requireCoordinates &&
    coordinateFilteredBusinesses.length === 0 &&
    transformedBusinesses.length > 0
  ) {
    console.warn(
      "[BUSINESSES API] For You requested map-ready results but unified set had no valid coordinates; using coordinate fallback.",
      {
        requestId: requestId ?? null,
  ...
  response.headers.set(
    "X-Feed-Path",
    requireCoordinates ? "for_you_unified_coords" : "for_you_unified"
  );
...
export async function fetchTopPicksFallback(
  ...
  const hasTaxonomyFilters =
    (subInterestIds && subInterestIds.length > 0) || (interestIds && interestIds.length > 0);

  let { data: rawData } = await buildFallbackQuery(true);
  if ((!rawData || rawData.length === 0) && hasTaxonomyFilters) {
    console.warn(
      "[BUSINESSES API] Top picks fallback returned 0 with preference filters; relaxing taxonomy filters.",
      {
        requestId: requestId ?? null,
  ...
  const transformed = cleanedRows.map(transformBusinessForCard).filter((business) => {

...
const variantStyles: Record<CardVariant, string> = {
  ...
  premium:
    "bg-off-white/95 border border-white/30 backdrop-blur-xl shadow-premiumElevated hover:shadow-premiumElevatedHover overflow-visible",
...
export const Card: React.FC<CardProps> = ({
  ...
  const hoverStyles =
    hoverable || clickable ? "hover:shadow-premiumHover hover:-translate-y-0.5" : "";
...
export const CardHeader: React.FC<{ className?: string; children: React.ReactNode }> = ({
  className = "",
  children,
}) => <div className={`pb-4 border-b border-light-gray/30 ${className}`}>{children}</div>;
...
export const CardFooter: React.FC<{ className?: string; children: React.ReactNode }> = ({
  className = "",
  children,
}) => <div className={`pt-4 border-t border-light-gray/30 ${className}`}>{children}</div>;

...
export const DangerAction: React.FC<DangerActionProps> = ({
  ...
  return (
    <div className={`font-urbanist ${showBorder ? "border-t border-coral/20 pt-4" : ""}`}>
      <h3 className={`text-base font-600 ${titleColor} mb-2`}>{title}</h3>
      <p className="text-sm text-charcoal/70 mb-4 font-semibold">{description}</p>
      <button type="button" onClick={onAction} className={`${buttonClasses} font-semibold`}>
        {buttonText}
      </button>
    </div>
  );

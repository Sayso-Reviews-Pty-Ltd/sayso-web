  ...
  return (
    <Link href={user.id ? `/reviewer/${user.id}` : "#"} className="w-full">
      <m.div
        ...
        className="group bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 rounded-[12px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.12)] cursor-pointer backdrop-blur-md hover:shadow-[0_12px_40px_rgb(0,0,0,0.18)] transition-shadow duration-300"
      >
        <div className="flex items-center justify-between p-3 sm:p-4">
          <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
            <div
              className={`w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br ${getBadgeStyles()} rounded-full flex items-center justify-center font-urbanist text-caption sm:text-body-sm font-600 shadow-[0_4px_12px_rgba(0,0,0,0.1)] flex-shrink-0`}
            >

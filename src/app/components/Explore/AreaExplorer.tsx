    ...
    <div className="space-y-4 font-urbanist">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <button
          ...
          className="group inline-flex items-center gap-1 text-body-sm sm:text-caption font-semibold text-charcoal transition-all duration-300 hover:text-sage focus:outline-none px-4 py-2 -mx-2 relative"
          aria-label="See all: Explore by Area"
        >
          <span className="relative z-10 transition-transform duration-300 group-hover:-translate-x-0.5 text-charcoal group-hover:text-sage font-semibold">
            See All
          </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
        {POPULAR_AREAS.map((area) => (
          <Link
            ...
            className="group relative bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 rounded-[12px] overflow-hidden backdrop-blur-md border-none shadow-md hover:shadow-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] p-4"
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-body-sm font-semibold text-charcoal mb-1 group-hover:text-navbar-bg transition-colors duration-300 truncate">
                  {area.name}
                </h3>
                <p className="text-caption text-charcoal/60 line-clamp-2">{area.description}</p>
              </div>
            </div>
          </Link>
        ))}

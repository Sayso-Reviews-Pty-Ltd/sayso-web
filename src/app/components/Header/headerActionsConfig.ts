...
export const DISCOVER_LINKS: readonly NavLink[] = [
  ...
  },
  {
    ...
    description: "Seasonal happenings & offers",
...
export const getMobileMenuActions = (isBusinessAccountUser: boolean): MobileMenuActionItem[] => {
  const commonActions: MobileMenuActionItem[] = [
    ...(isBusinessAccountUser
      ? [{ href: "/settings", label: "Settings", requiresAuth: true, delay: 2 }]
      : []),
  ];
...
export const getBusinessNavLinks = (
  ...
  if (hasOwnedBusinesses) {
    ...
    return BUSINESS_LINKS.filter(
      (link, idx, arr) => arr.findIndex((l) => l.key === link.key) === idx
 ...
 */
export const getLinkHref = (href: string, requiresAuth: boolean, isGuest: boolean): string => {
...
/** Typography uses Tailwind `font-urbanist` on `<header>` and dropdown surfaces. */
export const HEADER_FONT_STYLE = {} as const;

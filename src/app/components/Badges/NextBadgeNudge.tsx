...
export default function NextBadgeNudge({
  userId,
  compact = false,
  theme = "light",
}: NextBadgeNudgeProps) {
      ...
      <div className="flex flex-col gap-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="animate-pulse flex items-center gap-3">
            <div
              className={`h-9 w-9 rounded-lg shrink-0 ${isDark ? "bg-white/10" : "bg-charcoal/10"}`}
            />
            <div className="flex-1 space-y-1.5">
              <div className={`h-2.5 w-24 rounded ${isDark ? "bg-white/10" : "bg-charcoal/10"}`} />
              <div
                className={`h-2 w-full rounded-full ${isDark ? "bg-white/10" : "bg-charcoal/10"}`}
              />
            </div>
          </div>
        ))}
    ...
    <div className={`flex flex-col ${compact ? "gap-2" : "gap-4"}`}>
      {nearestBadges.map((badge, idx) => (
        ...
        >
          {/* Locked badge icon */}
          <div
            className={`relative shrink-0 h-9 w-9 ${isDark ? "opacity-65 brightness-110" : "grayscale opacity-50"}`}
          >
            <Image
              src={badge.iconPath}
              alt={badge.name}
          ...
          <div className="flex-1 min-w-0">
            {/* Label */}
            <p
              className={`font-urbanist text-xs font-600 leading-tight truncate ${isDark ? "text-white/85" : "text-charcoal"}`}
            >
            ...
            {/* Progress bar */}
            <div
              className={`mt-1 h-2 w-full rounded-full overflow-hidden ${isDark ? "bg-white/15 ring-1 ring-white/15" : "bg-charcoal/10"}`}
            >
              <m.div
            ...
            {!compact && (
              <p
                className={`mt-0.5 font-urbanist text-[10px] leading-none ${isDark ? "text-white/60" : "text-charcoal/50"}`}
              >

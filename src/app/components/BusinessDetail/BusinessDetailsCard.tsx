    ...
    if (typeof todayHours === "string") {
      ...
      if (match) {
        ...
        if (currentTime >= openTime && currentTime < closeTime) {
          return {
            isOpen: true,
            status: `Open until ${formatTime(`${closeHour}:${closeMin.toString().padStart(2, "0")}`)}`,
          };
        } else if (currentTime < openTime) {
          return {
            isOpen: false,
            status: `Opens at ${formatTime(`${openHour}:${openMin.toString().padStart(2, "0")}`)}`,
          };
        }
      }
    }
...
const parseHoursForDisplay = (
  hours: Hours
): { day: string; label: string; hours: string; isToday: boolean }[] | null => {
...
const formatPriceRange = (
  priceRange: string | { raw?: string; friendly?: string } | undefined
): { display: string; description: string } | null => {

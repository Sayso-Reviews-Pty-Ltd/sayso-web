...
export function ImageCarousel({
  ...
  const fallbackPlaceholder =
    placeholderImage ?? getSubcategoryPlaceholder(subcategorySlug ?? undefined);
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 rounded-[12px] bg-card-bg/90 px-3 sm:px-4 py-2 shadow-sm mx-auto text-center justify-center">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {metrics.map((m) => (
                <span
                  key={m.label}
                  className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-sm sm:text-xs ${
                    m.color === "sage"

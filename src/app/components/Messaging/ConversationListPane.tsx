...
export function ConversationListPane({
  ...
  return (
    <aside
      className={`${listPaneVisibleClass} w-full lg:w-[360px] xl:w-[400px] flex-col border-r border-charcoal/8 bg-off-white`}
    >
      {/* Header */}
      <div className="px-5 pb-3 pt-0">
        ...
        {role === "business" && businessOptions && businessOptions.length > 0 && (
          <div className="mt-3">
            <label className="sr-only" htmlFor="business-filter">
              Business filter
            </label>
            <select
              id="business-filter"
              ...
              className="w-full rounded-xl border-0 bg-charcoal/[0.06] px-3 py-2 text-sm text-charcoal focus:outline-none focus:ring-2 focus:ring-sage/25 font-urbanist"
            >
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            placeholder="Search"
            className="w-full rounded-full border-0 bg-charcoal/[0.06] py-2 pl-10 pr-4 text-sm text-charcoal placeholder:text-charcoal/40 focus:bg-charcoal/[0.09] focus:outline-none font-urbanist"
          />
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {(conversationsLoading || isResolvingStartConversation) && (
          <div className="flex h-full items-center justify-center">
            <div className="inline-flex items-center gap-2 text-sm text-charcoal/50 font-urbanist">
            ...
            <>
              {searchQuery.trim() ? (
                <div className="flex h-full flex-col items-center justify-center px-6 text-center">
                  ...
                  <p className="text-sm font-semibold text-charcoal/60 font-urbanist">
                ...
                <div className="flex h-full flex-col items-center justify-center px-5 py-8 text-center">
                  ...
                  <p className="text-base font-bold text-charcoal font-urbanist">
                    No conversations yet
                  </p>
                  {reviewedBusinessSuggestions.length > 0 ? (
                    <>
                      <p className="mt-1.5 text-sm text-charcoal/50 font-urbanist">
                        ...
                        {reviewedBusinessSuggestions.map((suggestion) => (
                          <li key={suggestion.business_id}>
                            ...
                            >
                              ...
                              <span className="flex-1 truncate text-sm font-semibold text-charcoal font-urbanist">
                    ...
                    <>
                      ...
                      <Link
                        href="/home"
                        className="mt-5 inline-flex items-center gap-2 rounded-full bg-navbar-bg px-5 py-2.5 text-sm font-bold text-white transition-colors hover:bg-navbar-bg/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-navbar-bg/40 font-urbanist"
                      >
                        Discover businesses
                      </Link>
                    </>
                ...
                <div className="flex h-full flex-col items-center justify-center px-6 text-center">
                  ...
                  <p className="mt-1 text-xs text-charcoal/40 font-urbanist">
        ...
        {!conversationsLoading && filteredConversations.length > 0 && (
          <ul className="py-2">
            {filteredConversations.map((conversation) => {
              ...
              const subtitleValue = getConversationSubtitle(
                conversation,
                role,
                fallbackBusinessName
              );
                ...
                <li key={conversation.id}>
                  ...
                  >
                    ...
                    <div className="relative flex-shrink-0">
                      ...
                      {hasUnread && (
                        <span
                          className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-navbar-bg"
                          aria-hidden
                        />
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-baseline justify-between gap-2">
                        <p
                          className={`truncate text-sm font-urbanist ${hasUnread ? "font-bold text-charcoal" : "font-semibold text-charcoal/85"}`}
                        >
                          {name}
                        </p>
                        <span className="flex-shrink-0 text-[11px] text-charcoal/35 font-urbanist">
                          {formatListTimestamp(conversation.last_message_at)}
                        </span>
                      </div>
                      <p
                        className={`mt-0.5 truncate text-xs font-urbanist ${hasUnread ? "font-semibold text-charcoal/70" : "text-charcoal/45"}`}
                      >

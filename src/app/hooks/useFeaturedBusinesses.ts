...
const mapLegacyBusinessToFeatured = (business: any, index: number): FeaturedBusiness => {
...
async function fetchFeaturedData(
  ...
  if (response.status === 404) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.warn(
        "[useFeaturedBusinesses] /api/featured returned 404, falling back to /api/businesses"
      );
    }
  ...
  if (usedLegacyFallback) {
    ...
    const normalized = fallbackList
      ...
      .map((item: unknown, index: number) =>
        mapLegacyBusinessToFeatured(item as Record<string, unknown>, index)
      );
    return {
      list: normalized,
      meta: {
...
export function useFeaturedBusinesses(
  options: UseFeaturedBusinessesOptions = {}
): UseFeaturedBusinessesResult {

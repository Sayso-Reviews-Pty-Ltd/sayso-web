...
async function fetcher(url: string): Promise<ConversationsResponse> {
  ...
  if (!response.ok) {
    ...
    const error: any = new Error(
      payload?.error || `Failed to fetch conversations (${response.status})`
    );
...
export function useConversations({ role, businessId, enabled = true }: UseConversationsOptions) {
  ...
  const conversations = useMemo(() => sortConversations(data?.data || []), [data?.data]);
          ...
          mutate(
            (previous) => {
              ...
              const existingIndex = previous.data.findIndex(
                (conversation) => conversation.id === row.id
              );
              if (existingIndex === -1) {
                void mutate();
                return previous;
              ...
              const updated = {
                ...
                last_message_at:
                  row.last_message_at || previous.data[existingIndex].last_message_at,

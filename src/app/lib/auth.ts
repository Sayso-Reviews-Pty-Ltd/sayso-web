...
export class AuthService {
  ...
  static async signUp({
    ...
    try {
      ...
      if (profileCheckError) {
        console.warn(
          "Profile check error (blocking signup to avoid duplicates):",
          profileCheckError
        );
        return {
          user: null,
  ...
  static async signIn({
    email,
    password,
  }: SignInData): Promise<{ user: AuthUser | null; error: AuthError | null }> {
    const supabase = this.getClient();
    try {
      if (!email?.trim() || !password?.trim()) {
        return {
  ...
  static async signOut(): Promise<{ error: AuthError | null }> {
    ...
    try {
      ...
      const globalSignOutPromise = supabase.auth
    ...
    try {
      ...
      if (error) {
        ...
        if (isNetworkError && retryCount < MAX_RETRIES) {
          console.warn(
            `AuthService: Network error, retrying (${retryCount + 1}/${MAX_RETRIES}):`,
            error.message
          );
      ...
      try {
        ...
        console.warn(
          "AuthService: Error fetching profile, continuing without profile:",
          profileError
        );
        profile = undefined;
      }
      ...
      if (isNetworkError && retryCount < MAX_RETRIES) {
        console.warn(
          `AuthService: Network error in catch, retrying (${retryCount + 1}/${MAX_RETRIES}):`,
          errorMessage
        );
      ...
      if (isSessionError) {
        ...
        if (
          ...
          console.warn(
            "AuthService: Session error in catch block (not signing out):",
            errorMessage
          );
        }
      } else {
  ...
  private static handleSupabaseError(error: { message: string; error_code?: string }): AuthError {
    ...
    if (
      ...
      (message.includes("email") &&
        ...
        (message.includes("use") || message.includes("taken") || message.includes("registered")))
    ) {
      return {
    ...
    if (
      ...
      return {
        message:
          "Email redirect URL is not configured correctly. Please contact support or try again later.",
    ...
    if (message.includes("422") || message.includes("unprocessable")) {
      return {
        message: "Request failed. Please check your details and try again.",
        code: "invalid_data",
      };
    }
      ...
      if (typeof window !== "undefined" && process.env.NEXT_PUBLIC_SITE_URL) {
        try {
          ...
          if (configuredOrigin !== runtimeOrigin) {
            console.warn(
              "[AuthService.resendVerificationEmail] NEXT_PUBLIC_SITE_URL origin differs from runtime origin",
              {
                configuredOrigin,
                runtimeOrigin,
              }

...
async function userOwnsBusiness(
  ...
  if (verifiedOwnerResult.error && verifiedOwnerResult.error.code !== "PGRST116") {
    console.error(
      "[createEventSpecial] verified ownership query error:",
      verifiedOwnerResult.error
    );
  }
...
export async function createEventOrSpecial(params: {
  ...
  const businessId = normalizeNullableString(
    forcedBusinessId ?? body.businessId ?? body.business_id
  );
  ...
  const ctaUrl = normalizeNullableString(
    body.ctaUrl ?? body.cta_url ?? body.bookingUrl ?? body.booking_url
  );
  const ctaLabel = normalizeNullableString(
    body.ctaLabel ?? body.cta_label ?? body.bookingContact ?? body.booking_contact
  );
  const rawWhatsappNumber = normalizeNullableString(body.whatsappNumber ?? body.whatsapp_number);
  const whatsappPrefillTemplate = normalizeNullableString(
    body.whatsappPrefillTemplate ?? body.whatsapp_prefill_template
  );
  ...
  if (usesWhatsapp && !whatsappNumber) {
    return {
      ok: false,
      status: 400,
      error: "WhatsApp number is required when booking method is WhatsApp",
    };
  }
  ...
  if (whatsappPrefillTemplate && whatsappPrefillTemplate.length > 2000) {
    return {
      ok: false,
      status: 400,
      error: "WhatsApp prefilled message must be 2000 characters or less",
    };
  }
  ...
  if (spamSignals.includes("contains_link") || spamSignals.includes("excessive_repetition")) {
    return {
      ok: false,
      status: 400,
      error: "Content failed spam checks. Remove links or repetitive text.",
    };
  }

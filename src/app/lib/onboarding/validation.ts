...
export function validateOnboardingPrerequisites(params: {
  ...
  if (params.requiredStep !== "interests" && (!params.interests || params.interests.length === 0)) {
...
export function validateSubcategoryIds(
  ...
  const invalidIds = subcategoryIds.filter((id) => !validSubcategoryIds.includes(id));
...
export function validateDealbreakerIds(
  ...
  const invalidIds = dealbreakerIds.filter((id) => !validDealbreakerIds.includes(id));

...
export function calculateComboScore(
  ...
  const distanceScore = distanceKm !== null ? 1 - normalize(distanceKm, 0, maxDistance) : 0.5; // Neutral if no distance

  // Rating score: higher is better
  const ratingScore = rating !== null && rating > 0 ? normalize(rating, 0, 5) : 0.3; // Lower score if no rating

  // Price score: cheaper is better (inverted)
  const priceScore = priceLevel !== null ? 1 - normalize(priceLevel, 1, 4) : 0.5; // Neutral if no price

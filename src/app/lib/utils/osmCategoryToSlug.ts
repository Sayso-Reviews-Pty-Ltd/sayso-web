...
export function getSubcategorySlugForOsmCategory(category: string, businessName?: string): string {
  ...
  if (businessName) {
    ...
    if (nameLower.includes("train station") || nameLower.includes("railway station")) {
      return "transport";
    }
    if (nameLower.includes("bus station") || nameLower.includes("bus terminal")) {
      return "transport";
    }
    if (nameLower.includes("car rental") || nameLower.includes("car hire")) {
      return "transport";
    }
    if (nameLower.includes("campervan")) {
      return "transport";
    }
    if (nameLower.includes("travel agency")) {
      return "travel-services";
    }
    if (nameLower.includes("tour guide")) {
      return "travel-services";
    }
    if (nameLower.includes("travel insurance")) {
      return "travel-services";
    }
    if (
  ...
  if (categoryMap[normalizedCategory]) {
    return (
      LEGACY_TRAVEL_SUBCATEGORY_MAP[categoryMap[normalizedCategory]] ??
      categoryMap[normalizedCategory]
    );
  }
  const lowerCategory = normalizedCategory.toLowerCase();
  const matchingKey = Object.keys(categoryMap).find((key) => key.toLowerCase() === lowerCategory);
  if (matchingKey) {

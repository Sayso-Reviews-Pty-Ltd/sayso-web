...
export async function fetchBusinessOptimized(
  ...
  const slugResult = await executeWithRetry(async () => {
  ...
  const [finalBusinessResult, reviewsResult, statsResult, businessImagesResult] =
      ...
      async () =>
        executeWithRetry(async () => {
          const { data, error } = await client2
            .from("reviews")
            .select(
              "id, user_id, business_id, rating, content, title, tags, created_at, helpful_count"
            )
  ...
  if (businessResult.error) {
    ...
    throw new Error(
      `Failed to fetch business: ${businessResult.error?.message || "Database error"}`
    );
  }
  ...
  if (reviewsResult.data && reviewsResult.data.length > 0) {
    const reviewIds: string[] = reviewsResult.data
      .map((r: any) => r.id)
      .filter((id): id is string => typeof id === "string");
    const userIdsArray = reviewsResult.data
      .map((r: any) => r.user_id)
      .filter((id): id is string => typeof id === "string");
  ...
  if (
    businessImagesResult.data &&
    Array.isArray(businessImagesResult.data) &&
    businessImagesResult.data.length > 0
  ) {
  ...
  const result = {
    ...businessResult.data,
    uploaded_images:
      uploadedImages.length > 0 ? uploadedImages : businessResult.data?.uploaded_images || [],

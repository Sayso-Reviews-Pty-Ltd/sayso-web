...
async function getCategoryOgImage(slug: string): Promise<string | undefined> {
  try {
    ...
    const businessImages = Array.isArray((data as any).business_images)
      ? (data as any).business_images
      : [];
    ...
    return orderedBusinessImages[0]?.url || (data as any).image_url || undefined;
  } catch (error) {
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  return (
    <>
      <SchemaMarkup
        schemas={[collectionSchema, itemListSchema, breadcrumbSchema, organizationSchema]}
      />

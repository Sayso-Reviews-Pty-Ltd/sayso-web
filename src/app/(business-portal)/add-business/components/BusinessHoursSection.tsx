...
const BusinessHoursSection: React.FC<BusinessHoursSectionProps> = ({ formData, onHoursChange }) => {
  ...
  const copyToWeekdays = useCallback(
    (sourceDay: string) => {
      ...
      daysConfig
        .filter((d) => d.isWeekday && d.key !== sourceDay)
        .forEach((day) => {
  ...
  return (
    <m.div
      className="relative bg-white rounded-[12px] overflow-hidden border border-charcoal/10 shadow-md px-4 py-6 sm:px-8 sm:py-8 md:px-10 md:py-10"
    ...
    >
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {hasAnyHours && (
            <m.span
              ...
              className="text-xs text-charcoal/60 bg-charcoal/5 px-3 py-1 rounded-full font-urbanist"
            >
        ...
        {/* Quick Presets */}
        <div className="mb-6">
          <p className="text-sm text-charcoal mb-3 flex items-center gap-2 font-urbanist">
            Quick setup
          </p>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`text-sm font-semibold font-urbanist ${isSelected ? "text-charcoal" : "text-charcoal/70"}`}
                    >
                      {preset.label}
                    </span>
                  </div>
                  <span className="text-xs text-charcoal/50 font-urbanist">
                    {preset.description}
                  </span>
                  {isSelected && (
                    <m.div
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm text-charcoal font-urbanist">Customize schedule</p>
            <button
              type="button"
              onClick={() => setShowCustomize(!showCustomize)}
              className="text-xs text-coral hover:text-coral/80 transition-colors flex items-center gap-1 font-urbanist"
            >
              ...
              return (
                <m.div
                  ...
                  className={`group relative flex items-center gap-3 p-3 rounded-xl transition-all duration-200 ${
                    isOpen ? "bg-gradient-to-r from-white/60 to-white/40" : "bg-white/30 opacity-60"
                  }`}
                >
                  {/* Day Toggle */}
                  <button
                  ...
                  <div className="w-20 flex-shrink-0">
                    <span
                      className={`text-sm font-semibold font-urbanist ${isOpen ? "text-charcoal" : "text-charcoal/50"}`}
                    >
                      {day.fullLabel}
                    </span>
                  </div>
                    ...
                    <AnimatePresence mode="wait">
                      {isOpen ? (
                        showCustomize ? (
                          ...
                          >
                            <input
                              ...
                              className="bg-white border border-charcoal/20 rounded-full px-3 py-1.5 text-sm text-charcoal font-semibold focus:outline-none focus:border-coral/50 focus:ring-2 focus:ring-coral/30 w-[100px] hover:border-sage/50 transition-all duration-200 font-urbanist"
                            />
                          ...
                          <m.div
                            ...
                            className="flex items-center gap-2"
                          >
                            <span className="text-sm text-charcoal/80 font-medium font-urbanist">
                        ...
                        <m.span
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="text-sm text-charcoal/45 italic font-urbanist"
                        >
                          Closed
                        </m.span>
                      )}
        ...
        {hasAnyHours && (
          ...
          >
            <button
              ...
              className="text-xs text-charcoal/70 hover:text-charcoal bg-charcoal/5 hover:bg-charcoal/10 px-3 py-1.5 rounded-full transition-all flex items-center gap-1.5 font-urbanist"
            >
            ...
            <button
              ...
              className="text-xs text-charcoal/45 hover:text-coral/80 px-3 py-1.5 rounded-full transition-all font-urbanist"
            >
              Clear all
            </button>
          </m.div>
        ...
        {/* Helper text */}
        {!hasAnyHours && (
          <p className="text-xs text-charcoal/45 mt-4 text-center font-urbanist">

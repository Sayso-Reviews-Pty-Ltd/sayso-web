  ...
  useEffect(() => {
    const updatePosition = () => {
      ...
      const maxHeight = Math.max(
        180,
        Math.min(360, Math.floor((prefersTop ? spaceAbove : spaceBelow) - 8))
      );
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <m.div animate={{ rotate: isOpen && !disabled ? 180 : 0 }} transition={{ duration: 0.2 }}>
          <ChevronDown size={16} />
        </m.div>
      </m.button>
          ...
          >
            ...
            >
              {filteredOptions.length === 0 && (
                <div
                  className="px-4 py-3 text-sm text-charcoal/70"
                  style={{
                    fontFamily:
                      "Urbanist, -apple-system, BlinkMacSystemFont, system-ui, sans-serif",
                  }}

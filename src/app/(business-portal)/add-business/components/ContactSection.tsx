        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onBlur={() => onBlur("phone")}
                className={`w-full bg-white/95 backdrop-blur-sm border pl-4 pr-4 py-3 sm:py-4 text-sm font-semibold text-charcoal placeholder-charcoal/50 placeholder:font-normal focus:outline-none focus:ring-2 transition-all duration-300 hover:border-sage/50 rounded-full font-urbanist ${
                  errors.phone
              ...
              {touched.phone && errors.phone && (
                <p className="mt-2 text-sm text-coral font-medium font-urbanist">{errors.phone}</p>
              )}
            </div>
            ...
            <div>
              ...
              {touched.email && errors.email && (
                <p className="mt-2 text-sm text-coral font-medium font-urbanist">{errors.email}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            {touched.website && errors.website && (
              <p className="mt-2 text-sm text-coral font-medium font-urbanist">{errors.website}</p>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="flex items-center gap-2 text-sm font-semibold text-charcoal font-urbanist">
            ...
            <div className="grid grid-cols-4 gap-2">
              {priceRangeOptions.map((option, index) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-5 h-5 sm:w-6 sm:h-6 ${isSelected ? "text-coral" : "text-charcoal/60"}`}
                    />
                    <span
                      className={`text-sm sm:text-base font-bold font-urbanist ${isSelected ? "text-charcoal" : "text-charcoal/70"}`}
                    >
                      {option.label}
                    </span>
                    <span className="text-[10px] sm:text-xs text-charcoal/50 hidden sm:block font-urbanist">
                      {option.description}
                    </span>
                    {isSelected && (
                      <m.div

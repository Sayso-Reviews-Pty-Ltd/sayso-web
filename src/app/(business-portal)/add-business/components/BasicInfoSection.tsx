...
import { Store, Loader2, Building2, Truck, Monitor, Link as LinkIcon } from "@/app/lib/icons";
...
const BasicInfoSection: React.FC<BasicInfoSectionProps> = ({
  ...
  const subcategoryOptions = useMemo(() => {
    ...
    const baseOptions = Array.from(dedupedMap.values()).sort((a, b) =>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            {nameDuplicateCheck.checking && (
              <p
                className="mt-2 text-sm text-charcoal/60 font-medium flex items-center gap-1.5 font-urbanist"
                aria-live="polite"
              >
            ...
            {touched.name && errors.name && !nameDuplicateCheck.checking && (
              <div id="name-error" role="alert" aria-live="polite" className="mt-2 space-y-1">
                <p className="text-sm text-coral font-medium font-urbanist">{errors.name}</p>
          ...
          <div>
            ...
            {loadingCategories ? (
              <div className="w-full bg-white/95 backdrop-blur-sm border-none pl-4 pr-4 py-3 sm:py-4 md:py-5 rounded-full flex items-center gap-2">
                ...
                <span className="text-body text-charcoal/60 font-semibold font-urbanist">
                  Loading categories...
                </span>
              </div>
            ) : (
              <>
                ...
                {selectedMainCategoryLabel && (
                  <p className="mt-2 text-xs text-charcoal/70 font-urbanist">
                    Selected: {selectedMainCategoryLabel}
                  </p>
                )}
                {formData.mainCategory === "miscellaneous" && (
                  <p className="mt-2 text-xs text-charcoal/80 font-urbanist">
                    We&apos;ll list this under Miscellaneous.
                  </p>
                )}
                {touched.mainCategory && errors.mainCategory && (
                  <p
                    id="mainCategory-error"
                    className="mt-2 text-sm text-navbar-bg font-medium font-urbanist"
                  >
                    {errors.mainCategory}
                  </p>
                )}
          ...
          <div>
            ...
            <CustomDropdown
              ...
              placeholder={
                formData.mainCategory ? "Select a subcategory" : "Select main category first"
              }
          ...
          {formData.category && (
            <div className="space-y-4 pt-4 border-t border-charcoal/10">
              <div>
                <label className="block text-sm font-semibold text-charcoal mb-3 font-urbanist">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  >
                    <Building2
                      className={`w-6 h-6 ${formData.businessType === "physical" ? "text-coral" : ""}`}
                    />
                  ...
                  >
                    <Truck
                      className={`w-6 h-6 ${formData.businessType === "service-area" ? "text-coral" : ""}`}
                    />
                  ...
                  >
                    <Monitor
                      className={`w-6 h-6 ${formData.businessType === "online-only" ? "text-coral" : ""}`}
                    />

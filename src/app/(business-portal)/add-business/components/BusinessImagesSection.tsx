        ...
        <div className="space-y-4">
          ...
          {imagePreviews.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {imagePreviews.map((preview, index) => (
                <div
                  key={index}
                  className="relative group aspect-square rounded-[16px] overflow-hidden bg-white/60 border border-charcoal/10"
                >
                  <Image
                    src={preview}
          ...
          <label className="block">
            <div className="w-full min-h-[120px] border-2 border-dashed border-charcoal/20 rounded-[16px] flex flex-col items-center justify-center gap-3 p-6 cursor-pointer hover:border-charcoal/30 hover:bg-off-white/70 transition-all duration-200">
              ...
              <div className="text-center">
                <span className="text-sm font-semibold text-charcoal block mb-1 font-urbanist">
          ...
          {uploadingImages && (
            <div className="flex items-center justify-center gap-2 py-4">
              ...
              <span className="text-sm text-charcoal/60 font-urbanist">Uploading images...</span>
            </div>
          )}
        </div>
      </div>

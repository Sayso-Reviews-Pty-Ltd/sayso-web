...
function DeltaBadge({ current, previous }: { current: number; previous: number }) {
  ...
  if (previous === 0) {
    return current > 0 ? (
      <span className="text-xs font-semibold text-emerald-600">New activity</span>
    ) : null;
  }
...
export default function OwnerBusinessDashboard() {
  ...
  const businessRouteId =
    ((business as any)?.slug as string | undefined) || business?.id || businessId;
    ...
    return [
      {
        ...
        done: Array.isArray(b.uploaded_images) && b.uploaded_images.length >= 5,
      },
      {
        label: "Complete your business description",
        done: !!b.description && b.description.length > 50,
      },
  ...
  useEffect(() => {
    if (realtimeReviews.length > 0 && analytics) {
      ...
      const recentReviewsCount = realtimeReviews.filter(
  ...
  const handleProfilePictureUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (business?.image_url && business.image_url.includes(STORAGE_BUCKETS.BUSINESS_IMAGES)) {
        try {
          ...
          if (pathIndex !== -1 && pathIndex < urlParts.length - 1) {
            ...
            await supabase.storage.from(STORAGE_BUCKETS.BUSINESS_IMAGES).remove([oldPath]);
          }
        } catch (deleteError) {
      ...
      if (updateError) {
        ...
        await supabase.storage.from(STORAGE_BUCKETS.BUSINESS_IMAGES).remove([filePath]);
        return;
      }
      ...
      <div className="font-urbanist">
        <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-5xl mx-auto">
          ...
          <div className="pt-1 pb-4 space-y-4">
            ...
            <section
              className="bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 backdrop-blur-xl rounded-[12px] shadow-lg p-5 sm:p-6 space-y-4"
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-5xl mx-auto">
        ...
        <div className="pt-1 pb-4">
          <div className="space-y-4">
            ...
            <article className="w-full sm:mx-0" aria-labelledby="business-heading">
            ...
            >
              ...
              <div className="bg-gradient-to-br from-card-bg via-card-bg to-card-bg/80 backdrop-blur-xl border-none rounded-[12px] shadow-lg p-4 min-h-[120px] flex flex-col justify-between hover:shadow-md transition-shadow duration-200">
                ...
                <div>
                  ...
                  <div className="mt-0.5">
                    <DeltaBadge
                      current={analytics?.newReviews ?? 0}
                      previous={analytics?.prevReviews ?? 0}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-card-bg via-card-bg to-card-bg/80 backdrop-blur-xl border-none rounded-[12px] shadow-lg p-4 min-h-[120px] flex flex-col justify-between hover:shadow-md transition-shadow duration-200">
                ...
                <div>
                  ...
                  <div className="mt-0.5">
                    <DeltaBadge
                      current={analytics?.profileViews ?? 0}
                      previous={analytics?.prevProfileViews ?? 0}
                    />
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <div className="mt-0.5">
                    <DeltaBadge
                      current={analytics?.newConversations ?? 0}
                      previous={analytics?.prevConversations ?? 0}
                    />
                  </div>
                </div>
              </Link>
            ...
            <section aria-label="Quick actions" className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                <Link
                  href={`/my-businesses/${businessRouteId}/reviews`}
                  className="group bg-gradient-to-br from-card-bg via-card-bg to-card-bg/80 backdrop-blur-xl border-none rounded-[12px] p-4 flex flex-col items-center gap-2 text-center hover:border-sage/40 hover:translate-y-[-2px] transition-all duration-200"
                >
                  <span className={`${ICON_CHIP_LARGE_CLASS} group-hover:bg-off-white/90`}>
                ...
                <div className="bg-gradient-to-br from-card-bg via-card-bg to-card-bg/80 backdrop-blur-xl border-none rounded-[12px] p-4 flex flex-col items-center gap-2 text-center opacity-60 cursor-default">
                  <span className={ICON_CHIP_LARGE_CLASS}>
                    <TrendingUp className="w-5 h-5" />
                  </span>
                  <span className="text-sm font-semibold text-charcoal/60">Promote</span>
                  <span className="text-[10px] bg-card-bg/20 text-navbar-bg px-1.5 py-0.5 rounded-full font-semibold">
                    Soon
                  </span>
                </div>
                <Button
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-semibold text-charcoal/70 tabular-nums">
                  {profileCompletion}%
                </span>
              </div>
                  ...
                  <li key={item.label} className="flex items-center gap-3">
                    <span
                      className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                        item.done
                          ? "bg-off-white/90 text-charcoal/85"
                          : "bg-off-white/70 text-charcoal/60"
                      }`}
                    >
                      {item.done ? (
                    ...
                    <span
                      className={`text-sm ${item.done ? "text-charcoal/50 line-through" : "text-charcoal/80 font-medium"}`}
                    >
                      {item.label}
                    </span>
                  </li>

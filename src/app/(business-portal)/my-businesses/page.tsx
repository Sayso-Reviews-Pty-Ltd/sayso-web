...
import { Store, CalendarDays, Sparkles, Clock3, MapPin, Plus } from "@/app/lib/icons";
...
export default function MyBusinessesPage() {
  ...
  const fetchOwnerListings = useCallback(
    async (ownedBusinesses: Business[], userId: string): Promise<OwnerListingsFetchResult> => {
          ...
          ...ownedBusinesses.map(async (business) => {
            ...
            try {
              const response = await fetch(
                `/api/businesses/${business.id}/events?owner_view=true`,
                {
                  cache: "no-store",
  ...
  const filteredOwnerListings = useMemo(
    () =>
      ownerListings.filter((l) => {
        const matchesType = listingsTypeFilter === "all" || l.type === listingsTypeFilter;
        const matchesBusiness =
          listingsBusinessFilter === "all" || l.businessId === listingsBusinessFilter;
    ...
    <>
      ...
      <div className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        <Breadcrumb className="pt-4 sm:pt-6 pb-2">
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbPage>My Businesses</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        {(businesses.length > 0 || ownerListings.length > 0) && (
          <div
            className={
              businesses.length > 0
                ? "grid grid-cols-1 lg:grid-cols-12 lg:gap-6"
                : "flex justify-center"
            }
          >
            {/* Businesses table */}
            ...
            >
              <div className="rounded-2xl border border-charcoal/10 bg-white shadow-premium flex flex-col min-h-0 overflow-hidden">
                ...
                <div className="flex-shrink-0 px-4 pt-3">
                  <div
                    className="grid grid-cols-3 w-full rounded-xl bg-charcoal/5 p-1 gap-0.5"
                    role="tablist"
                  >
                    {(
                      [
                      ...
                      return (
                        <button
                          ...
                          className={`px-2 py-1.5 rounded-lg text-xs font-semibold transition-colors font-urbanist ${
                            active
                              ? "bg-navbar-bg text-white shadow-sm"
                ...
                <div className="flex-1 min-h-0 overflow-y-auto p-4 pt-3">
                  {ownerListings.length === 0 ? (
                    <div className="rounded-xl border border-charcoal/8 bg-charcoal/[0.025] px-4 py-8 text-center">
                      <p className="font-urbanist text-sm font-semibold text-charcoal/60">
                        No events or specials yet.
                      </p>
                      <p className="font-urbanist text-xs text-charcoal/40 mt-1">
                      ...
                      {filteredOwnerListings.map((listing) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold font-urbanist ${
                                listing.type === "event"
                                  ? "bg-navbar-bg/10 text-navbar-bg"

...
function ClaimBusinessPageContent() {
  ...
  const getClaimStatusBadge = (displayStatus: string, status: string) => {
    ...
    if (status === "verified") {
      return (
        <span className={`${base} bg-card-bg/15 text-sage border-sage/30`}>
    ...
    if (displayStatus === "Under Review") {
      return (
        <span className={`${base} bg-amber-50 text-amber-800 border-amber-200`}>
          <FileCheck className="w-3.5 h-3.5" /> Under Review
        </span>
      );
    }
    if (displayStatus === "Action Required") {
      return (
        <span className={`${base} bg-coral/10 text-coral border-coral/30`}>
          <AlertCircle className="w-3.5 h-3.5" /> Action Required
        </span>
      );
    }
    return (
      <span className={`${base} bg-charcoal/10 text-charcoal/80 border-charcoal/20`}>
            ...
            <div className="pt-6 sm:pt-8 pb-12">
              <div className="max-w-[800px] mx-auto">
                ...
                <div className="py-12 text-center">
                  <span
                    className={`${ICON_CHIP_CLASS} w-14 h-14 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 shadow-sm`}
                  >
                  ...
                  >
                    Claim your business profile to respond to reviews, update information, and
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                          <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                            <span
                              className={`${ICON_CHIP_CLASS} w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0 group-hover:bg-off-white/90`}
                            >
                          ...
                          <div className="flex-shrink-0">{getActionButton(business)}</div>
                        </div>
                      </div>
                    ))}
                </div>
                ...
                <div className="mt-8 sm:mt-12 mb-8 sm:mb-12 p-4 sm:p-6 bg-white border border-charcoal/10 rounded-[12px] shadow-sm">
                  ...
                  >
                    Our business verification process is quick and easy. You'll need to provide

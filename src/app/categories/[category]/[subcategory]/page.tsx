...
function matchesSubcategory(business: any, subcategorySlug: string): boolean {
  const candidates = [
    ...
    business?.primary_subcategory_label
      ? toSlug(String(business.primary_subcategory_label))
      : undefined,
  ]
    .filter(Boolean)
...
export async function generateMetadata({
  ...
  try {
    ...
    const { data } = await supabase
      .from("businesses")
      .select(
        "image_url, business_images(url, is_primary, sort_order), primary_subcategory_slug, sub_interest_id, primary_subcategory_label"
      )
    ...
    if (match) {
      const businessImages = Array.isArray((match as any).business_images)
        ? (match as any).business_images
        : [];
      ...
      ogImage = orderedBusinessImages[0]?.url || (match as any).image_url || undefined;
    }
  } catch (error) {
...
export default async function CategorySubcategoryPage({ params }: CategorySubcategoryPageProps) {
  ...
  const { data: businesses, error } = await supabase
    .from("businesses")
    .select(
      "id, name, slug, description, image_url, location, primary_category_slug, primary_subcategory_slug, primary_subcategory_label, sub_interest_id, average_rating:business_stats(average_rating), business_images(id, url, type, sort_order, is_primary)"
    )
    .eq("primary_category_slug", category)
    .eq("status", "active")
    .or("is_system.is.null,is_system.eq.false")
  ...
  const filteredBusinesses = (businesses || []).filter((business) =>
    matchesSubcategory(business, subcategory)
  );

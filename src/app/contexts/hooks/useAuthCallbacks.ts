        ...
        if (authUser) {
          ...
          if (desiredRole && authUser.profile) {
            ...
            if (!hasDesiredRole) {
              ...
              setError(
                `This email only has a ${existingTypeName} account. Please select ${existingTypeName} to log in, or register a new ${accountTypeName} account.`
              );
              setIsLoading(false);
          ...
          if (typeof window !== "undefined") {
            try {
              ...
              if (candidate && candidate.startsWith("/") && !candidate.startsWith("//")) {
                ...
                const isForbidden = forbiddenTargets.some(
    ...
    async (userData?: Partial<AuthUser>): Promise<void> => {
      ...
      try {
        ...
        let { data: freshProfile, error: fetchError } = await supabase
          .from("profiles")
          .select(
            "user_id, onboarding_step, onboarding_complete, onboarding_completed_at, interests_count, last_interests_updated, created_at, updated_at, avatar_url, username, display_name, is_top_reviewer, reviews_count, badges_count, subcategories_count, dealbreakers_count"
          )
          .eq("user_id", user.id)
          .single();
        ...
        if (userData && userData.profile) {
          transformedProfile = {
            username:
              userData.profile.username !== undefined
                ? userData.profile.username?.trim() || undefined
                : user.profile?.username,
            display_name:
              userData.profile.display_name !== undefined
                ? userData.profile.display_name?.trim() || undefined
                : user.profile?.display_name,
            avatar_url:
              userData.profile.avatar_url !== undefined
  ...
  const resendVerificationEmail = useCallback(
    async (
      ...
      try {
        ...
        if (user?.email_verified && user.email?.trim().toLowerCase() === normalizedEmail) {
          return {
            success: false,

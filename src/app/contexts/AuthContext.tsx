...
import {
  ...
  subscribeAuthLifecycleEvent,
} from "../lib/authLifecycle";
import {
  AUTH_DEBOUNCE_MS,
  LOCALSTORAGE_CLEANUP_DELAY_MS,
  DEFAULT_AUTH_CONTEXT,
} from "./AuthContext.constants";
...
export function AuthProvider({
  children,
  initialSnapshot = UNKNOWN_AUTH_SNAPSHOT,
}: AuthProviderProps) {
  ...
  const [isAuthInitialized, setIsAuthInitialized] = useState(
    initialSnapshot.status === "authenticated"
  );
    ...
    const initializeAuth = async () => {
      ...
      const attemptInit = async (attempt: number): Promise<void> => {
        ...
        try {
          ...
          console.error(
            `[AuthContext] Error initializing auth (attempt ${attempt}/${MAX_RETRIES}):`,
            error
          );
  ...
  const { login, register, logout, updateUser, refreshUser, resendVerificationEmail } =
    useAuthCallbacks(
      user,
      setUser,
      setIsLoading,

...
import { Edit, Star, ChevronUp, Info, ChevronRight, ChevronLeft } from "@/app/lib/icons";
...
import BusinessInfoModal, {
  BusinessInfo,
} from "../../../components/BusinessInfo/BusinessInfoModal";
...
function WriteReviewContent() {
  ...
  const businessId =
    (params?.id as string) || searchParams.get("business_id") || searchParams.get("businessId");
  ...
  } = useReviews(business?.id ? actualBusinessId : undefined);
    ...
    async function fetchReviewForEdit() {
      ...
      try {
        ...
        if (review.review_images && Array.isArray(review.review_images)) {
          ...
          const imageUrls = review.images.filter((url: string) => url && url.trim() !== "");
  ...
  const businessImages = useMemo(() => {
    ...
    if (
      ...
      allImages = business.uploaded_images.filter(
        (url: string) =>
          url && typeof url === "string" && url.trim() !== "" && !isPlaceholderImage(url)
      );
    }
    ...
    if (
      ...
      !allImages.includes(business.image_url)
    ) {
    ...
    if (
      ...
      !allImages.includes(business.image)
    ) {
    ...
    const optimisticReview = {
      ...
      profile: user
        ? {
            user_id: user.id,
            display_name:
              (user as any).display_name || (user as any).user_metadata?.display_name || null,
      ...
      user: {
        id: user?.id || "",
        name: user
          ? (user as any).display_name || (user as any).user_metadata?.display_name || "You"
          : "Anonymous",
        avatar_url: (user as any)?.avatar_url || (user as any)?.user_metadata?.avatar_url || null,
      },
      review_images: [],
    ...
    return (
      <div className="min-h-dvh bg-off-white flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          ...
          <p className="text-body text-charcoal/70 mb-6 font-urbanist">
                ...
                <ol className="flex items-center gap-2 text-sm sm:text-base">
                  <li>
                    <Link
                      ...
                      className="text-charcoal/70 hover:text-charcoal transition-colors duration-200 font-medium truncate max-w-[200px] sm:max-w-none font-urbanist"
                    >
                      {businessName}
                    </Link>
                  </li>
                  ...
                  <li>
                    <span className="text-charcoal font-semibold font-urbanist">Write Review</span>
                  </li>
                </ol>
              </nav>
                  ...
                  <div className="grid gap-6 lg:grid-cols-3 items-start">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                      ...
                      <BusinessCarousel
                        ...
                        subcategorySlug={business?.sub_interest_id ?? business?.subInterestId}
                      />

                      {/* Review Form Section - Separated from images */}
                      <article
                        className="w-full sm:mx-0 flex items-center justify-center"
                  ...
                  <section className="space-y-6 pt-12" aria-labelledby="what-others-saying-heading">
                    <div className="flex justify-center">
                      <div className="flex flex-col gap-3">
                        <h2 id="what-others-saying-heading" className="sr-only">
                          What Others Are Saying
                        </h2>
                        <WavyTypedTitle
                          text="What Others Are Saying"
                          as="h2"
                          className="font-urbanist text-2xl sm:text-3xl md:text-4xl font-bold text-charcoal border-b border-charcoal/10 pt-4 pb-2"
                          typingSpeedMs={40}
                          startDelayMs={300}
                          waveVariant="subtle"
                          loopWave={true}

...
export async function generateMetadata({
  ...
  try {
    ...
    const { data } = await supabase
      .from("businesses")
      .select(
        "name, description, image_url, business_images(url, is_primary, sort_order), slug, primary_category_slug, primary_subcategory_label, category_raw, business_stats(average_rating, total_reviews)"
      )
      .eq("id", actualId)
      .eq("status", "active")
      .or("is_system.is.null,is_system.eq.false")
...
export default async function BusinessLayout({ children, params }: BusinessLayoutProps) {
  ...
  try {
    ...
    const hoursToStrings = (h: Record<string, string> | null): string[] => {
      ...
      return Object.entries(h)
        .filter(
          ([, v]) => v && typeof v === "string" && v.toLowerCase() !== "closed" && v.trim() !== ""
        )
    ...
    if (business) {
      ...
      const citySlug = business.location ? toSlug(String(business.location).split(",")[0]) : "";
      ...
      const businessSchema = generateLocalBusinessSchema({
        name: business.name,
        description:
          business.description || `${categoryLabel} located in ${business.location || "Cape Town"}`,
        ...
        openingHours: hoursToStrings((business as any).hours),
        aggregateRating:
          business.business_stats?.[0]?.average_rating &&
          (business.business_stats?.[0]?.total_reviews || 0) > 0
            ? {
    ...
    <>
      ...
      {businessSummary && (
        <article aria-label="Business summary" className="sr-only">
          ...
          <p>
            {businessSummary.category} in {businessSummary.location}
          </p>
          {businessSummary.rating !== null && (
            <p>
              Rated {businessSummary.rating.toFixed(1)} out of 5 from {businessSummary.reviewCount}{" "}
              review{businessSummary.reviewCount === 1 ? "" : "s"}.
            </p>
          )}
        </article>
      )}

...
export async function generateMetadata({ params }: CityPageProps): Promise<Metadata> {
  ...
  return generateSEOMetadata({
    ...
    keywords: [
      displayCityName,
      categoryName?.toLowerCase() || "businesses",
      "cape town reviews",
      "sayso",
    ],
    url: `/${slug}`,
...
export default async function CityPage({ params }: CityPageProps) {
  ...
  let query = supabase
    .from("businesses")
    .select(
      "id, name, slug, description, image_url, location, primary_subcategory_slug, primary_subcategory_label, average_rating:business_stats(average_rating), business_images(id, url, type, sort_order, is_primary)"
    )
  ...
  const itemListSchema = generateItemListSchema(
    categoryName
      ? `Best ${categoryName}s in ${displayCityName}`
      : `Best Businesses in ${displayCityName}`,

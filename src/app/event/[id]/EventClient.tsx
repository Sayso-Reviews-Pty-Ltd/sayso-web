...
import { H1, H3, P, Muted } from "@/app/components/ui/typography";
...
export default function EventDetailPage({ params }: EventDetailPageProps) {
  ...
  const isNotFound =
    errorStatus === 404 || (event !== null && event.type !== "event" && event.type !== "special");
  ...
  const hasDirectCta =
    Boolean(
      event?.bookingUrl ||
      event?.purchaseUrl ||
      (event as any)?.ticketmaster_url ||
      (event as any)?.url
    ) ||
    (((event?.ctaSource ?? "").toLowerCase() === "whatsapp" || Boolean(event?.whatsappNumber)) &&
      Boolean(event));
    ...
    return (
      <div className="min-h-dvh bg-off-white flex items-center justify-center">
        <div className="text-center p-6">
          ...
          <H1 className="text-2xl font-bold text-charcoal mb-4">Event Not Found</H1>
          <Link
    ...
    return (
      <div className="min-h-dvh bg-off-white flex items-center justify-center">
        <div className="text-center p-6">
          ...
          <H1 className="text-2xl font-bold text-charcoal mb-4">Unable to Load Event</H1>
          <P className="text-charcoal/70 mb-6">{loadError || "Please try again."}</P>
          <Link
            href="/events-specials"
            className="font-urbanist px-6 py-2.5 bg-gradient-to-br from-charcoal to-charcoal/90 text-white rounded-full text-sm font-600 hover:bg-charcoal/90 transition-all duration-300 border border-white/30 inline-block"
          >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
                          ...
                          <ul className="space-y-2">
                            {ticketRows.slice(0, 6).map((ticket, index) => (
                              <li
                                ...
                                className="rounded-[12px] bg-off-white/55 p-3"
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    ...
                                    {ticket.description && (
                                      <Muted className="mt-0.5 text-xs text-charcoal/70">
                                        {ticket.description}
                                      </Muted>
                                    )}
                                  </div>
                    ...
                    {scheduleRows.length > 0 && (
                      <div className="bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 backdrop-blur-xl border-none rounded-[12px] shadow-md p-4 sm:p-6">
                        <div className="mb-3 flex items-center justify-between gap-3">
                          <H3 className="text-h3 font-semibold text-charcoal">Event Schedules</H3>
                          <span className="font-urbanist inline-flex items-center rounded-full bg-off-white/70 px-2.5 py-1 text-xs font-semibold text-charcoal/75">
                            {scheduleRows.length}
                          </span>
                        </div>
                        ...
                        <ul className="space-y-2.5">
                          {visibleScheduleRows.map((schedule, index) => (
                            ...
                            >
                              ...
                              <Muted className="mt-1 text-xs text-charcoal/70">
                                {formatScheduleDateTime(schedule.startDate)}
                                {schedule.endDate
                                  ? ` → ${formatScheduleDateTime(schedule.endDate)}`
                                  : ""}
                              </Muted>
                            </li>
                          ))}
                        ...
                        {scheduleRows.length > 3 && (
                          <button
                            ...
                            className="font-urbanist mt-3 inline-flex items-center gap-1.5 rounded-full bg-off-white/70 px-3 py-1.5 text-xs font-semibold text-charcoal/80 transition-colors duration-200 hover:bg-off-white"
                          >
                            {showAllSchedules ? (
                              <>
                    ...
                    {(() => {
                      ...
                      const currentEnd =
                        event.endDateISO || event.endDate
                          ? new Date(event.endDateISO || event.endDate!)
                          : null;
                      ...
                      const otherDates = occurrencesList.reduce<
                            ...
                            {otherDates.map((o) => {
                              ...
                              return (
                                <li key={o.id} className="flex items-center justify-between gap-3">
                                  <span className="font-urbanist text-body-sm text-charcoal/80">
                                    {o.label}
                                  </span>
                                  <Link
                                    href={`${hrefBase}/${o.id}`}
                                    className="font-urbanist text-body-sm font-semibold text-coral hover:underline"
                                  >
                                    View
                                  </Link>
                                </li>
                  ...
                  <div className="space-y-4 sm:space-y-6">
                      ...
                      <div className="bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 backdrop-blur-xl border-none rounded-[12px] shadow-md p-4 sm:p-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {categoryNames.map((category) => (
                            <span
                              key={category}
                              className="font-urbanist inline-flex items-center rounded-full bg-off-white/70 px-3 py-1.5 text-xs font-semibold text-charcoal/80"
                            >
                              {category}
                            </span>
                          ))}
                    ...
                    <EventPersonalizationInsights
                      event={{ id: event.id, rating: liveRating, totalReviews: liveTotalReviews }}
                    />
          ...
          <section className="mx-auto w-full max-w-[2000px] px-2 relative z-10 mt-8 pb-8">
            ...
            {relatedEvents.length > 0 && (
              <div className="mt-6 bg-gradient-to-br from-card-bg via-card-bg to-card-bg/95 backdrop-blur-xl border-none rounded-[12px] shadow-md p-4 sm:p-6">
                <H3 className="text-h3 font-semibold text-charcoal mb-4">
                    ...
                    <li key={rel.id}>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <P className="text-sm font-semibold text-charcoal group-hover:text-navbar-bg transition-colors line-clamp-1">
                            {rel.title}
                          </P>
                          <Muted className="text-xs text-charcoal/60 line-clamp-1">
                            {rel.startDate}
                            {rel.city ? ` · ${rel.city}` : ""}
                          </Muted>
                        </div>
                        {rel.availabilityStatus && (
                          <span
                            className={`font-urbanist flex-shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full ${

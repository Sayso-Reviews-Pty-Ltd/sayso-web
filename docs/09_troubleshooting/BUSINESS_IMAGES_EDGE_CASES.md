...
if (imagesError) {
  ...
  for (const url of uploadedUrls) {
    ...
    if (pathMatch?.[1]) {
      await supabase.storage.from("business-images").remove([pathMatch[1]]);
    }
  }
}
```
...
async function cleanupOrphanedImages() {
  const { data: images } = await supabase.from("business_images").select("id, url");
...
if (uploadError) {
  if (uploadError.message?.includes("quota") || uploadError.message?.includes("storage limit")) {
    showToast(
      "Storage quota exceeded. Please contact support or delete old images.",
      "error",
      5000
    );
  } else {

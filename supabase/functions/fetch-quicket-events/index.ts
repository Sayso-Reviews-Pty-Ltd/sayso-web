...
function getTicketStats(tickets: QuicketTicket[] | null | undefined) {
  ...
  const available = nonDonation.filter(
    (t) => t.soldOut !== true && t.provisionallySoldOut !== true
  );
  const availablePriced = available
    .map((t) => t.price)
    .filter((p): p is number => isFiniteNum(p) && p > 0);
...
function normalizeDetailPayload(payload: unknown): QuicketEvent | null {
  ...
  if (Array.isArray(data.results) && data.results.length > 0)
    return data.results[0] as QuicketEvent;
...
function mapEvent(
  ...
  return {
    ...
    guestlist_count:
      isFiniteNum(event.guestListCount) && event.guestListCount! > 0 ? event.guestListCount! : 0,
  };
}
...
function consolidate(rows: EventRow[]): EventRow[] {
  ...
  for (const row of rows) {
    ...
    if (row.end_date && ex.end_date && new Date(row.end_date) > new Date(ex.end_date))
      ex.end_date = row.end_date;
    else if (row.end_date && !ex.end_date) ex.end_date = row.end_date;
    if (row.description && (!ex.description || row.description.length > ex.description.length))

-- Fix upsert_events_and_specials_consolidated to update guestlist_count on conflict.
-- Previously guestlist_count was omitted from DO UPDATE SET, so existing events
-- would never have their attendance counts refreshed by the Quicket cron.
...
BEGIN
    ...
    SELECT
      ...
      CASE
        WHEN nullif(btrim(r->>'quicket_category_slug'), '') IN (
          'music', 'festivals', 'tech-business', 'arts', 'food-drink', 'community'
        )
  ...
  upserted AS (
    INSERT INTO public.events_and_specials (
      title, type, business_id, start_date, end_date, location, description,
      icon, image, price, rating, booking_url, booking_contact, created_by,
      is_community_event, external_url, cta_source, whatsapp_number,
      whatsapp_prefill_template, availability_status, quicket_category_slug,
      quicket_category_label, quicket_event_id, event_name, event_description,
      event_url, event_image_url, event_start_date, event_end_date,
      event_created_at, event_last_modified, venue_id, venue_name,
      venue_address_line1, venue_address_line2, venue_latitude, venue_longitude,
      locality_level_one, locality_level_two, locality_level_three, organiser_id,
      organiser_name, organiser_phone, organiser_mobile, organiser_facebook_url,
      organiser_twitter_handle, organiser_hashtag, organiser_page_url,
      quicket_categories_json, tickets_json, minimum_ticket_price,
      maximum_ticket_price, tickets_available_boolean, schedules_json, guestlist_count
    )
    SELECT
      f.title, f.type, f.business_id, f.start_date, f.end_date, f.location,
      f.description, f.icon, f.image, f.price, f.rating, f.booking_url,
      f.booking_contact, f.created_by, f.is_community_event, f.external_url,
      f.cta_source, f.whatsapp_number, f.whatsapp_prefill_template,
      f.availability_status, f.quicket_category_slug,
      coalesce(f.quicket_category_label, CASE f.quicket_category_slug
        WHEN 'music' THEN 'Music' WHEN 'festivals' THEN 'Festivals'
        WHEN 'tech-business' THEN 'Tech & Business' WHEN 'arts' THEN 'Arts'
        WHEN 'food-drink' THEN 'Food & Drink' WHEN 'community' THEN 'Community'
        ELSE NULL END),
      f.quicket_event_id,
      ...
      f.event_end_date, f.event_created_at, f.event_last_modified,
      f.venue_id, f.venue_name, f.venue_address_line1, f.venue_address_line2,
      f.venue_latitude, f.venue_longitude, f.locality_level_one,
      f.locality_level_two, f.locality_level_three, f.organiser_id,
      f.organiser_name, f.organiser_phone, f.organiser_mobile,
      f.organiser_facebook_url, f.organiser_twitter_handle, f.organiser_hashtag,
      f.organiser_page_url, f.quicket_categories_json, f.tickets_json,
      f.minimum_ticket_price, f.maximum_ticket_price, f.tickets_available_boolean,
      f.schedules_json, f.guestlist_count
    FROM filtered f
    ON CONFLICT (
      (lower(btrim(title))),
    ...
    DO UPDATE SET
      ...
      -- Always take the fresher guestlist_count from the source API.
      -- Use GREATEST so a non-zero API value always wins over a stale 0.
      guestlist_count               = GREATEST(EXCLUDED.guestlist_count, 0),
      updated_at                    = now()

...
async function checkBadgeSystem() {
  ...
  try {
    ...
    const { data: reviews, error: reviewError } = await supabase.from("reviews").select("user_id");
    ...
    const userReviewCounts = Object.entries(reviewStats)
      .map(([user_id, count]) => ({
        user_id,
        count,
      }))
    ...
    if (problemUsers.length > 0) {
      ...
      for (const user of problemUsers.slice(0, 10)) {
        // Show first 10
        const userId = user.userId.substring(0, 8) + "...";
        console.log(
          `│ ${userId.padEnd(7)} │ ${user.reviewCount.toString().padStart(11)} │ ${user.currentBadges.toString().padStart(10)} │ ${"New Voice".padEnd(20)} │`
        );
      }
      console.log("└─────────┴─────────────┴────────────┴──────────────────────┘");

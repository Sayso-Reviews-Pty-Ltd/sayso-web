...
async function verifyBucket(bucketName: string): Promise<VerificationResult> {
  ...
  try {
    ...
    if (!bucket) {
      result.errors.push(
        `Bucket "${bucketName}" does not exist. Create it in Supabase Dashboard > Storage.`
      );
      return result;
    }
    ...
    if (!result.isPublic) {
      result.errors.push(
        `Bucket "${bucketName}" is not public. Set it to public in Supabase Dashboard.`
      );
    }
...
async function main() {
  ...
  const bucketsToCheck = ["business-images", "review_images"];
